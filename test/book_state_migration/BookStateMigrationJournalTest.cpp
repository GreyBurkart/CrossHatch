...
class TestIo {
 ...
 private:
  ...
  static Status removeThunk(void* context, const char* path) { return static_cast<TestIo*>(context)->remove(path); }
...
#define DOMAIN_CALLBACK(name, action) \
  static Status name##Thunk(void* context, const Record&) { return static_cast<Domain*>(context)->run(action); }
...
#define DURABLE_CALLBACK(name, action) \
  static Status name##Thunk(void* context, const Record&) { return static_cast<DurableDomain*>(context)->run(action); }
...
TEST(BookStateMigrationCodec, ExactGoldenAndStrictTruncationAndCrcValidation) {
  ...
  const std::vector<uint8_t> golden{0x42, 0x4d, 0x4a, 0x31, 0x01, 0x00, 0x28, 0x00, 0x04, 0x03, 0x02, 0x01, 0x01, 0x02,
                                    0xa5, 0x00, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x18, 0x17, 0x16, 0x15,
                                    0x14, 0x13, 0x12, 0x11, 0x09, 0x00, 0x0e, 0x00, 0x17, 0x00, 0x00, 0x00, 0x2f, 0x6f,
                                    0x6c, 0x64, 0x2e, 0x65, 0x70, 0x75, 0x62, 0x2f, 0x52, 0x65, 0x61, 0x64, 0x2f, 0x6e,
                                    0x65, 0x77, 0x2e, 0x65, 0x70, 0x75, 0x62, 0xd2, 0x2d, 0xbc, 0xa2};
...
TEST(BookStateMigrationJournal, EveryTornInactiveWriteAndDurabilityFaultKeepsOlderSelection) {
  ...
  for (const FaultPoint point : {FaultPoint::Open, FaultPoint::Write, FaultPoint::Flush, FaultPoint::Sync,
                                 FaultPoint::Close, FaultPoint::Read}) {
    Fixture fixture;
    Selection selected{};
    ASSERT_EQ(fixture.journal.begin(fixture.input(), &selected), Status::Ok);
...
TEST(BookStateMigrationRecovery, RebootAfterEveryDurablePhaseAndFallbackSwitchesOnlyAtActivation) {
  ...
  const std::array<Phase, 7> expected{Phase::SourceMoved,     Phase::CacheCopied,   Phase::BookmarksCopied,
                                      Phase::ClippingsCopied, Phase::StateVerified, Phase::Activated,
                                      Phase::OldStateRemoved};
...
TEST(BookStateMigrationRecovery, StatefulDurableModelPreservesExactOrderAndActivationBoundary) {
  ...
  const std::array<Phase, 7> phases{Phase::SourceMoved,     Phase::CacheCopied,   Phase::BookmarksCopied,
                                    Phase::ClippingsCopied, Phase::StateVerified, Phase::Activated,
                                    Phase::OldStateRemoved};
...
TEST(BookStateMigrationRecovery, EveryDomainFaultLeavesPhaseRetryableWithoutLaterMutation) {
  const std::array<Domain::Action, 13> actions{Domain::Action::Rename,
                                               Domain::Action::CopyCache,
                                               Domain::Action::VerifyCache,
                                               Domain::Action::CopyBookmarks,
                                               Domain::Action::VerifyBookmarks,
                                               Domain::Action::CopyClippings,
                                               Domain::Action::VerifyClippings,
                                               Domain::Action::VerifyState,
                                               Domain::Action::ActivateRecent,
                                               Domain::Action::ActivateOpenPath,
                                               Domain::Action::VerifyActivation,
                                               Domain::Action::RemoveOldState,
                                               Domain::Action::None};
