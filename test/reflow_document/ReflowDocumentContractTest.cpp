...
class FakeReflowDocument final : public ReflowDocument {
 public:
  ...
  std::string getCoverBmpPath(const bool cropped = false, const bool = false) const override {
    return cropped ? cachePath_ + "/cover_cropped.bmp" : cachePath_ + "/cover.bmp";
  }

  bool generateCoverBmp(const bool cropped = false, const GfxRenderer* = nullptr, const int = 0,
