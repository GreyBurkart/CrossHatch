...
TEST(PdfLayoutWordIndexTest, ReadsValidatedPageRecordsInOneBoundedFourRecordIo) {
  ...
  for (uint32_t ordinal = 0; ordinal < 4; ++ordinal) {
    ASSERT_TRUE(writer.append(
        range(ordinal, ordinal, ordinal, "b00000000"),
        PdfLayoutPageRecord{ordinal + 1U, static_cast<uint16_t>(ordinal), static_cast<uint16_t>(ordinal + 10U)}));
  }
  ASSERT_TRUE(writer.finish());
  ...
  source.resetReadCount();
  EXPECT_EQ(pdfReadValidatedLayoutPageRecords(source.source(), info, 0, 5, pages).error, PdfError::InvalidArgument);
...
TEST(PdfReaderProgressStateTest, UnresolvedExactRestoreRejectsMatchingVisibleRangeWithoutMutatingTupleOrRetryState) {
  ...
  EXPECT_FALSE(state.cacheRelayoutCapture(&state.currentPageSemanticRange, &sidecarRangeC, &sidecarRangeC));
