...
std::vector<uint8_t> wideCodeSpaceSimpleFontPdf() {
  static constexpr std::string_view cmap =
      ...
      "end\n"
      "end";
  static constexpr std::string_view content = "BT /F1 12 Tf 1 0 0 1 72 720 Tm (Corporate) Tj ET";
  return classicPdf({
      "<< /Type /Catalog /Pages 2 0 R >>",
      "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
      "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] "
...
std::vector<uint8_t> multiScalarWideCodeSpaceSimpleFontPdf() {
  static constexpr std::string_view cmap =
      ...
      "end\n"
      "end";
  static constexpr std::string_view content = "BT /F1 12 Tf 1 0 0 1 72 720 Tm <4469651F6E> Tj ET";
  return classicPdf({
      "<< /Type /Catalog /Pages 2 0 R >>",
      "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
      "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] "
...
std::vector<uint8_t> moreThanSixteenUsedFontsPdf() {
  ...
  resources += " >> >>";
  content += "ET";
  objects[2] = "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources " + resources + " /Contents 4 0 R >>";
...
std::vector<uint8_t> largeUnusedWidthsWithToUnicodePdf() {
  ...
  return classicPdf({
      ...
      "/Widths " +
          widths + " /FontDescriptor 7 0 R /ToUnicode 6 0 R >>",
      streamObject(cmap),
...
std::vector<uint8_t> unobservedToUnicodePdf() {
  static constexpr std::string_view content = "BT /F1 12 Tf 1 0 0 1 72 720 Tm <48656C6C6F> Tj ET";
...
std::vector<uint8_t> denseCidWithUnobservedMalformedFontPdf(const bool appendEmptyStrings = false) {
  ...
  for (size_t index = 0; index < kDenseCidChunkCount; ++index) {
    ...
    content += "/Span << /ActualText (" + std::string(replacement, static_cast<size_t>(replacementLength)) +
               ") >> BDC BT /F1 12 Tf 1 0 0 1 72 " + std::to_string(760U - index * 8U) + " Tm <" + encodedGlyphs +
               "> Tj ET EMC\n";
  }
...
std::vector<uint8_t> cidUniqueGlyphPdf(const size_t glyphCount, const bool addUnpaintedAlias = false) {
  ...
  for (size_t start = 0; start < glyphCount; start += 50U) {
    ...
    for (size_t index = 0; index < count; ++index) {
      char code[8]{};
      const int codeLength = std::snprintf(code, sizeof(code), "%04X", static_cast<unsigned>(start + index));
      if (codeLength != 4) {
        return {};
      }
...
std::vector<uint8_t> sixteenPaintedFontsSpilledJournalPdf() {
  ...
  for (size_t font = 0; font < kSixteenFontCount; ++font) {
    ...
    resources += " /" + std::string(name, static_cast<size_t>(nameLength)) + " " + std::to_string(5U + font) + " 0 R";
    objects.emplace_back("<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>");
  }
  resources += " >> >>";
  ...
  for (size_t font = 0; font < kSixteenFontCount; ++font) {
    ...
    for (size_t record = 0; record < kSixteenFontRecordsPerFont; ++record) {
      content += "/Span << /ActualText (A) >> BDC BT /" + std::string(name, static_cast<size_t>(nameLength)) +
                 " 12 Tf 1 0 0 1 72 720 Tm <";
  ...
  objects[2] = "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] " + resources + " /Contents 4 0 R >>";
  objects[3] = streamObject(content);
  return classicPdf(objects);
}
...
std::vector<uint8_t> largeSpilledCMapSingleTokenPdf() {
  ...
  for (size_t code = 0; code < glyphCount; ++code) {
    ...
    if (std::snprintf(source, sizeof(source), "%02X", static_cast<unsigned>(code)) != 2 ||
        std::snprintf(destination, sizeof(destination), "%04X", static_cast<unsigned>(0x0100U + code)) != 4) {
      return {};
    }
  ...
  const std::string content =
      "/Span << /ActualText (Bounded) >> BDC BT /F1 12 Tf 1 0 0 1 72 720 Tm <" + encoded + "> Tj ET EMC";
  return classicPdf({
      "<< /Type /Catalog /Pages 2 0 R >>",
      "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
      "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] "
...
std::vector<uint8_t> maximumSortedCMapPdf() {
  ...
  for (size_t code = 0; code < mappingCount; ++code) {
    ...
    if (std::snprintf(source, sizeof(source), "%04X", static_cast<unsigned>(code)) != 4 ||
        std::snprintf(destination, sizeof(destination), "%04X", static_cast<unsigned>(0x2000U + code)) != 4) {
      return {};
    }
    cmap += "<" + std::string(source) + "> <" + std::string(destination) + ">\n";
  ...
  cmap +=
      ...
      "end\n ";
  const std::string content =
      "/Span << /ActualText (Maximum) >> BDC BT /F1 12 Tf 1 0 0 1 72 720 Tm <" + encoded + "> Tj ET EMC";
  return classicPdf({
      "<< /Type /Catalog /Pages 2 0 R >>",
      "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
      "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] "
...
std::vector<uint8_t> formResourceScopePdf() {
  ...
  static constexpr std::string_view formContent = "BT /F1 12 Tf 1 0 0 1 72 20 Tm <41> Tj ET";
  return classicPdf({
      "<< /Type /Catalog /Pages 2 0 R >>",
      "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
      "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] "
...
PdfWithTargetSpan locateTargetSpan(std::vector<uint8_t> pdf, const std::string_view marker, const size_t targetLength) {
...
PdfWithTargetSpan repeatedFormReadPdf() {
  ...
  const std::string formBody = std::string(marker) + "BT /F1 12 Tf 1 0 0 1 72 20 Tm (DecodedOnce) Tj ET";
  std::vector<uint8_t> pdf = classicPdf({
      "<< /Type /Catalog /Pages 2 0 R >>",
      "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
      "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] "
...
std::vector<uint8_t> nestedFormDepthPdf(const size_t depth) {
  ...
  objects[2] =
      "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] "
      "/Resources << /Font << /F1 " +
      std::to_string(fontObject) + " 0 R >> /XObject << /First 5 0 R >> >> /Contents 4 0 R >>";
...
std::vector<uint8_t> formResourceCapPdf(const size_t count) {
  ...
  for (size_t index = 0; index < count; ++index) {
    ...
    objects[4U + index] = streamObject(
        "BT /F1 12 Tf 1 0 0 1 72 10 Tm (Cap" + std::string(name + 1, static_cast<size_t>(nameLength - 1)) + ") Tj ET",
...
struct CancelOnSequentialCMapReadHarness {
  ...
  static PdfStatus read(void* const context, const PdfCacheHandle handle, const uint64_t offset,
    ...
    const PdfStatus status =
        harness->backing.read(harness->backing.context, handle, offset, destination, requested, bytesRead);
    ...
    if (!isFontStorePath(observation.path) || harness->preparation->phase() != PdfPreparationPhase::ParseFonts) {
      return status;
    }
  ...
  static PdfStatus metadata(void* const context, const PdfCacheHandle handle, PdfCacheFileMetadata* const value) {
    auto* const harness = self(context);
  ...
  PdfPreparationConfig config(const char* const path) {
    ...
    return {wrapped,
            ...
            {&base, PreparationHarness::measure, nullptr},
            rename,
            800,
            480};
  }
...
std::string preparedSection(const PdfPreparation& preparation, const PreparationHarness& harness) {
  const std::string path = std::string(preparation.cacheRoot()) + "/gen_" + std::to_string(preparation.generation()) +
                           "/sections/000000.xhtml";
...
ProductObservation prepareProduct(const char* const sourcePath, const std::vector<uint8_t>& pdf) {
  ...
  observation.terminal = runToTerminal(preparation, harness, &observation.terminalEntryPhase, &readTrace);
  ...
  const std::string fontStorePath =
      std::string(preparation.cacheRoot()) + "/gen_" + std::to_string(preparation.generation()) + "/build.font";
...
size_t readsStartingInTargetSpan(const ProductObservation& observation, const std::string_view sourcePath,
  ...
  return static_cast<size_t>(std::count_if(observation.sourceReads.begin(), observation.sourceReads.end(),
                                           [sourcePath, &pdf, targetEnd](const PdfTestReadObservation& read) {
                                             return read.path == sourcePath && read.offset >= pdf.targetOffset &&
                                                    read.offset < targetEnd;
                                           }));
}
...
::testing::AssertionResult completedWithSection(const ProductObservation& observation) {
  ...
  if (!observation.terminal.complete()) {
    ...
    const size_t firstEvent = observation.terminalEvents.size() > 12U ? observation.terminalEvents.size() - 12U : 0U;
...
TEST(PdfPreparationProductGapRed, PreservesMultiScalarLigaturesInWideSimpleFontCMap) {
  const ProductObservation observation =
      prepareProduct("/books/product-gap-wide-simple-ligature.pdf", multiScalarWideCodeSpaceSimpleFontPdf());
...
TEST(PdfPreparationProductGapRed, UsesSimpleFontEncodingWhenToUnicodeIsPartial) {
  const ProductObservation observation =
      prepareProduct("/books/product-gap-partial-tounicode.pdf", partialToUnicodeWithWinAnsiFallbackPdf());
...
TEST(PdfPreparationProductGapRed, DoesNotSpendGlyphWorkspaceOnBuiltInFallbackMappings) {
  const ProductObservation observation =
      prepareProduct("/books/product-gap-common-glyphs.pdf", moreThan256FallbackEquivalentGlyphsPdf());
...
TEST(PdfPreparationProductGapRed, IgnoresUnusedLargeWidthArrayAndKeepsToUnicode) {
  const ProductObservation observation =
      prepareProduct("/books/product-gap-large-widths.pdf", largeUnusedWidthsWithToUnicodePdf());
...
TEST(PdfPreparationProductGapRed, AcceptsFullPaintedGlyphBudgetBeforeLaterUnpaintedAlias) {
  ...
  const ProductObservation observation = prepareProduct("/books/product-gap-256-cid-plus-unpainted.pdf", pdf);
...
TEST(PdfPreparationProductGapRed, UsesSimpleEncodingWhenPaintedCodeMissingFromCMap) {
  const ProductObservation observation = prepareProduct("/books/product-gap-missing-cmap.pdf", missingCMapMappingPdf());
...
TEST(PdfPreparationProductGapRed, SuppressesHiddenOcrDuplicateAndKeepsPlacedImage) {
  ...
  const auto imageOpen = std::find_if(
      observation.terminalEvents.begin(), observation.terminalEvents.end(),
      [](const std::string& event) { return event.starts_with("open:") && event.ends_with("/build.image-files"); });
  const auto imageClose = std::find_if(
      observation.terminalEvents.begin(), observation.terminalEvents.end(),
      [](const std::string& event) { return event.starts_with("close:") && event.ends_with("/build.image-files"); });
  ASSERT_NE(imageOpen, observation.terminalEvents.end());
  const auto pageRecordsOpen = std::find_if(
      std::next(imageOpen), observation.terminalEvents.end(),
      [](const std::string& event) { return event.starts_with("open:") && event.ends_with("/build.page-records"); });
...
TEST(PdfPreparationProductGapRed, ResolvesNestedFormFromTheCallingFormsLocalXObjectDictionary) {
  const ProductObservation observation = prepareProduct("/books/product-gap-nested-form.pdf", nestedFormPdf());
...
TEST(PdfPreparationProductGapRed, RejectsAProductFormCycleByObjectIdentity) {
  const ProductObservation observation = prepareProduct("/books/product-gap-form-cycle.pdf", cyclicFormPdf());
...
TEST(PdfPreparationProductGapRed, AcceptsExactFormDepthAndOmitsOnlyTheDeeperBranch) {
  const ProductObservation exact =
      prepareProduct("/books/product-gap-form-depth-exact.pdf", nestedFormDepthPdf(PdfLimits::MaxFormDepth));
  ...
  const ProductObservation over =
      prepareProduct("/books/product-gap-form-depth-over.pdf", nestedFormDepthPdf(PdfLimits::MaxFormDepth + 1U));
...
TEST(PdfPreparationProductGapRed, AcceptsExactPerScopeFormResourceCapAndOmitsOnlyTheExcessForm) {
  const ProductObservation exact = prepareProduct("/books/product-gap-form-cap-exact.pdf",
  ...
  const ProductObservation over = prepareProduct("/books/product-gap-form-cap-over.pdf",
...
TEST(PdfPreparationProductGapRed, ResolvesXrefStreamAndCompressedObjectStreamIntoCommittedXhtml) {
  ...
  ASSERT_TRUE(completedWithSection(observation));
  EXPECT_EQ(observation.words, 4U);
  EXPECT_NE(observation.section.find(">Compressed object stream text.</p>"), std::string::npos) << observation.section;
}
