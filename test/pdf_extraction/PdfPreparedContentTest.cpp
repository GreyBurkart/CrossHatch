...
struct DefaultFont {
  ...
  DefaultFont()
      : cmap(cmapBuffer.data(), cmapBuffer.size(), {cmapRecords.data(), static_cast<uint16_t>(cmapRecords.size())}),
  ...
  void initialize(const uint16_t mappedA = 0x03A9) {
    ...
    const std::string cmapText =
        "1 begincodespacerange <00> <FF> endcodespacerange 1 beginbfchar <41> <" + std::string(mapping) + "> endbfchar";
...
struct GeneratedByteSource {
  ...
  static PdfStatus readAt(void* const context, const uint64_t offset, uint8_t* const destination,
    ...
    const size_t available =
        static_cast<size_t>(std::min<uint64_t>(generated.length - offset, std::numeric_limits<size_t>::max()));
...
struct CountingByteStore {
  ...
  static PdfStatus readAt(void* const context, const uint64_t offset, uint8_t* const destination,
    ...
    const size_t available =
        static_cast<size_t>(std::min<uint64_t>(counting.length - offset, std::numeric_limits<size_t>::max()));
    *bytesRead = std::min(requested, available);
...
struct InlineImageReplayHarness {
  ...
  static PdfStepResult finish(void* const context, const PdfByteSource& source, const uint64_t idEndOffset,
                              PdfWorkBudget& budget, uint64_t* const resumeOffset, PdfContentXObject* const image) {
...
TEST(PdfPreparedContentStreams, DecodesRawAndFlateStreamsIntoStableDeclaredRanges) {
  ...
  static constexpr std::array<uint8_t, 35> secondCompressed{
      0x78, 0xda, 0x73, 0x0a, 0x51, 0xd0, 0x77, 0x33, 0x54, 0x30, 0x34, 0x52, 0x08, 0x49, 0x53, 0xd0, 0x08, 0x4e,
      0x4d, 0xce, 0xcf, 0x4b, 0xd1, 0x54, 0x08, 0xc9, 0x52, 0x70, 0x0d, 0x01, 0x00, 0x5d, 0x00, 0x07, 0x1e,
  };
  static constexpr char second[] = "BT /F1 12 Tf (Second) Tj ET";

  PdfTestByteSource firstInput(std::vector<uint8_t>(reinterpret_cast<const uint8_t*>(first),
  ...
  PdfPreparedContentStreams prepared({sourceBuffer.data(), sourceBuffer.size(), outputBuffer.data(),
                                      outputBuffer.size(), dictionary.data(), dictionary.size()});
...
TEST(PdfPreparedContentStreams, AppendsMoreThanSixteenStreamsAsOneSeparatedSequence) {
  ...
  ASSERT_TRUE(prepared.beginSequence(batch.data(), PdfPreparedContentStreams::MaxSources, decodedStore.store()).ok());
  ...
  ASSERT_TRUE(prepared.beginSequenceAppend(batch.data(), overflowStreams, decodedStore.store(), firstBatchBytes).ok());
...
TEST(PdfPreparedContentStreams, DecoderOriginatedCancellationUsesTheSameBoundedCleanup) {
  ...
  PdfTestByteSource input(std::vector<uint8_t>(reinterpret_cast<const uint8_t*>(content),
...
TEST(PdfPreparedContentStreams, NeverAllowsCallerToRelaxGlobalExpansionCeilings) {
  static constexpr std::array<uint8_t, 90> highRatioFlate{
      ...
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe0, 0x2b, 0x0f, 0x6b, 0xad, 0x6f, 0xbb,
  };
  const PdfStreamDecodeLimits relaxed{std::numeric_limits<uint64_t>::max(), std::numeric_limits<uint16_t>::max()};
...
TEST(PdfPreparedContentResources, DirectDescriptorRemainsStableAcrossCooperativeInterpreterSteps) {
  static_assert(
      std::is_same_v<decltype(std::declval<PdfPreparedContentResources&>().descriptor()), const PdfContentResources&>);
...
TEST(PdfPreparedContentResources, ForwardsSharedInlineImageReplayHookWithoutDictionaryTokenCallback) {
  ...
  const PdfStepResult result =
      descriptor.finishInlineImage(descriptor.context, source, 17, budget, &resumeOffset, &image);
...
TEST(PdfPreparedContentResources, FormWithoutOwnResourcesInheritsCallingDescriptor) {
  DefaultFont defaultFont;
  defaultFont.initialize();
  PdfTestByteSource formInput(std::vector<uint8_t>{'B', 'T', ' ', '/', 'F', '1', ' ', '1', '2', ' ', 'T',
                                                   'f', ' ', '(', 'A', ')', ' ', 'T', 'j', ' ', 'E', 'T'});
  std::array<PdfPreparedFontResource, 1> fonts{};
  std::array<PdfPreparedXObjectResource, 1> xobjects{};
  PdfPreparedContentResources resources({fonts.data(), fonts.size(), xobjects.data(), xobjects.size()});
...
TEST(PdfPreparedContentResources, ImageResourceMakesHiddenOcrQualificationAndDeduplicationObservable) {
  ...
  const PdfHiddenTextContext context{
      ...
      storage.model.runs(),
      storage.model.runCount(),
      storage.model.text(),
      storage.model.textLength(),
      storage.model.images(),
      storage.model.imageCount(),
  };
...
TEST(PdfPreparedContentResources, RejectsUnboundedOrUnstableRegistration) {
  ...
  std::array<PdfPreparedXObjectResource, 1> xobjects{};
  PdfPreparedContentResources oversized({tooManyFonts.data(), tooManyFonts.size(), xobjects.data(), xobjects.size()});
  ...
  EXPECT_EQ(bounded.addFont(reinterpret_cast<const uint8_t*>("F1"), 2, nullptr).error, PdfError::InvalidArgument);
  EXPECT_EQ(bounded
                .addFont(reinterpret_cast<const uint8_t*>("font-name-that-is-deliberately-too-long"), 39,
...
TEST(PdfPreparedContentResources, RejectsFontThatWouldReadSpillDuringInterpretation) {
  ...
  EXPECT_EQ(resources.addFont(reinterpret_cast<const uint8_t*>("F1"), 2, &spilledFont).error, PdfError::LimitExceeded);
...
TEST(PdfPreparedContentResources, MaterializesOnlyPageUsedGlyphsFromLargeSpilledSourceMap) {
  ...
  PdfCMap sourceCmap(cmapBuffer.data(), cmapBuffer.size(),
                     {residentCmapRecords.data(), static_cast<uint16_t>(residentCmapRecords.size()), cmapSpill.store(),
                      &cmapReader, selectRecordReader});

  std::string cmapText = "1 begincodespacerange <00> <FF> endcodespacerange 64 beginbfchar ";
...
TEST(PdfPreparedContentResources, RejectsMoreThanGlobalPageGlyphBudgetAcrossFontSlices) {
  ...
  EXPECT_EQ(resources.addFont(reinterpret_cast<const uint8_t*>("F3"), 2, &pageFont3).error, PdfError::LimitExceeded);
