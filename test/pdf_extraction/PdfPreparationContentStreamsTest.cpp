...
struct PdfPreparationTestAccess {
  ...
  static uint32_t snapshotOffset(const PdfPreparation& preparation) { return preparation.inlineNavigationSpoolOffset_; }
  ...
  static LiveNavigationShape liveNavigationShape(const PdfPreparation& preparation) {
    return {
        preparation.fontNavigationSnapshotBytes_,       preparation.fontNavigationPageWindowBytes_,
        preparation.fontNavigationLinkCount_,           preparation.fontNavigationXObjectCount_,
        preparation.fontNavigationPageLabelCount_,      preparation.fontNavigationImageCacheEntryCount_,
        preparation.fontNavigationImageCandidateCount_,
    };
  }
  static std::vector<uint8_t> liveNavigationBytes(PdfPreparation& preparation, const LiveNavigationShape shape) {
    ...
    while (offset < shape.snapshotBytes) {
      ...
      uint8_t* const source =
          preparation.fontNavigationSnapshotFieldBytes(preparation.navigation_, offset, &contiguous);
  ...
  static bool rasterRuntimeActive(const PdfPreparation& preparation) { return preparation.rasterRuntimeActive_; }
  static bool sectionRepairRuntimeActive(const PdfPreparation& preparation) {
    return preparation.sectionRepairRuntimeActive_;
  }
  static uint64_t failedRasterImages(const PdfPreparation& preparation) { return preparation.failedRasterImages_; }
  ...
  static size_t runtimePrefixBytes(PdfPreparation& preparation) {
    size_t contiguous = 0;
    uint8_t* const tail = preparation.preparedNavigationSpillBytes(PdfLimits::PageTextBytes, &contiguous);
  ...
  static size_t placementPrefixBytes(PdfPreparation& preparation) {
    ...
    uint8_t* const operand =
        preparation.preparedNavigationSpillBytes(PdfLimits::PageTextBytes + runTailBytes, &operandBytes);
...
std::vector<uint8_t> onePagePdf(const std::string& contents, const std::vector<std::string>& contentObjects) {
...
void appendXrefEntry(std::string* const output, const uint8_t type, const uint32_t field2, const uint16_t field3) {
...
TwoPageObjectStreamFixture twoPageObjectStreamPdf() {
  static constexpr std::string_view firstContent = "BT /F1 12 Tf 72 720 Td (First object stream page.) Tj ET";
  static constexpr std::string_view secondContent = "BT /F1 12 Tf 72 720 Td (Second object stream page.) Tj ET";
  ...
  appendObject(9, streamObjectWithDictionary(
                      encodedXref, "/Type /XRef /Size 10 /Root 1 0 R /W [1 4 2] /Index [0 10] /Filter /FlateDecode"));
  pdf += "startxref\n" + std::to_string(xrefOffset) + "\n%%EOF\n";

  return {{pdf.begin(), pdf.end()},
          static_cast<uint32_t>(decodedObjectStream.size()),
          static_cast<uint32_t>(firstContent.size() + secondContent.size()),
          static_cast<uint32_t>(xrefData.size())};
}

std::vector<uint8_t> twoPagePdf(const std::string& firstContentObject, const std::string& secondContentObject) {
...
PdfStepResult stepWithPublicBounds(PdfPreparation& preparation, PreparationHarness& harness, const uint32_t step) {
  const uint32_t operationsBefore = harness.storage.operationCalls();
  const uint64_t bytesReadBefore = harness.storage.bytesReadTotal();
  const uint64_t bytesWrittenBefore = harness.storage.bytesWrittenTotal();
...
bool advanceToPhase(PdfPreparation& preparation, PreparationHarness& harness, const PdfPreparationPhase phase,
                    uint32_t* const step) {
...
std::string contentStorePath(const PdfPreparation& preparation) {
  return std::string(preparation.cacheRoot()) + "/gen_" + std::to_string(preparation.generation()) + "/build.content";
}

std::string objectStorePath(const PdfPreparation& preparation) {
  return std::string(preparation.cacheRoot()) + "/gen_" + std::to_string(preparation.generation()) + "/build.objstm";
}
...
TEST(PdfPreparationContentStreams, AccountsAndRetiresObjectStreamStoreBeforeContentDecode) {
  ...
  EXPECT_EQ(
      std::count(harness.storage.removeObservations().begin(), harness.storage.removeObservations().end(), storePath),
      1);
...
TEST(PdfPreparationContentStreams, DecodesFlateContentBeforeWritingSemanticXhtml) {
  ...
  EXPECT_EQ(std::find_if(events.begin(), events.end(),
                         [](const std::string& event) { return event.find("build.content-nav") != std::string::npos; }),
            events.end());
}
...
TEST(PdfPreparationContentStreams, ConcatenatesContentsArrayInDeclaredOrder) {
  ...
  const std::vector<uint8_t> pdf = onePagePdf("[4 0 R 5 0 R]", {streamObject(first), streamObject(second)});
  ASSERT_FALSE(pdf.empty());
...
TEST(PdfPreparationContentStreams, EnforcesExactDocumentCapForRawAndFilteredStreams) {
  ...
  for (const bool filtered : {false, true}) {
    for (const bool oneByteOver : {false, true}) {
      ...
      const std::vector<uint8_t> pdf = onePagePdf("4 0 R", {streamObject(encoded, filtered ? "FlateDecode" : nullptr)});
      ASSERT_FALSE(pdf.empty());
      ...
      const uint32_t baseline =
          static_cast<uint32_t>(PdfLimits::MaxExpandedRequiredStreamBytes - content.size() + (oneByteOver ? 1U : 0U));
...
TEST(PdfPreparationContentStreams, RestoresV2DocumentBudgetAndRejectsV1ResumeRecord) {
  ...
  const std::vector<uint8_t> pdf = twoPagePdf(streamObject(first), streamObject(zlibStored(second), "FlateDecode"));
  ASSERT_FALSE(pdf.empty());
  ...
  const std::string journalPath =
      std::string(interrupted.cacheRoot()) + "/gen_" + std::to_string(generation) + "/resume.journal";
...
TEST(PdfPreparationContentStreams, ChecksFreshFreeSpaceAndCleansFailedTemporaryStore) {
  ...
  const std::vector<uint8_t> pdf = onePagePdf("4 0 R", {streamObject(zlibStored(content), "FlateDecode")});
  ASSERT_FALSE(pdf.empty());
...
TEST(PdfPreparationContentStreams, RoundTripsLiveNavigationStateWithoutTouchingRuntime) {
  ...
  EXPECT_EQ(copyBytes(PdfPreparationTestAccess::runRecords(preparation), runtimePrefixBytes), runtimeBeforeSuspend);
  ...
  EXPECT_EQ(copyBytes(PdfPreparationTestAccess::runRecords(preparation), runtimePrefixBytes), runtimeBeforeRestore);
  EXPECT_EQ(copyBytes(PdfPreparationTestAccess::operandScratch(preparation), placementPrefixBytes),
    ...
    for (uint32_t cancelStep = 0; cancelStep < 512U; ++cancelStep) {
      ...
      if (storeRemovedStep == UINT32_MAX &&
          std::find(harness.storage.removeObservations().begin() + static_cast<std::ptrdiff_t>(removeStart),
                    ...
                    storePath) != harness.storage.removeObservations().end()) {
