...
uint32_t pdfSetupIoOperationDelta(const PdfSetupIoSnapshot& before, const PdfSetupIoSnapshot& after) {
  return after.open - before.open + after.read - before.read + after.write - before.write + after.flush - before.flush +
         after.sync - before.sync + after.close - before.close + after.remove - before.remove + after.rename -
         before.rename + after.mkdir - before.mkdir + after.list - before.list + after.capacity - before.capacity +
...
bool pdfHasSingleTruncatingOpen(const std::vector<PdfTestOpenObservation>& observations, const std::string& path) {
  size_t matching = 0;
...
std::string pdfSetupIoTraceLine(const PdfSetupIoSnapshot& before, const PdfSetupIoSnapshot& after,
  ...
  std::snprintf(
      counts, sizeof(counts), "o%lu r%lu w%lu f%lu s%lu c%lu d%lu n%lu m%lu l%lu p%lu t%lu @%u %c",
      static_cast<unsigned long>(after.open - before.open), static_cast<unsigned long>(after.read - before.read),
      static_cast<unsigned long>(after.write - before.write), static_cast<unsigned long>(after.flush - before.flush),
      static_cast<unsigned long>(after.sync - before.sync), static_cast<unsigned long>(after.close - before.close),
      static_cast<unsigned long>(after.remove - before.remove),
      static_cast<unsigned long>(after.rename - before.rename), static_cast<unsigned long>(after.mkdir - before.mkdir),
...
std::vector<uint8_t> makeTwoPageTextPdf(const bool unreadableFirst = false, const bool includeThird = false,
  ...
  const std::string firstStream = unreadableFirst ? "q Q" : "BT /F1 12 Tf 72 720 Td (First durable page) Tj ET";
  const std::string secondStream =
      secondPageContent != nullptr ? secondPageContent : "BT /F1 12 Tf 72 720 Td (Second resumed page) Tj ET";
...
std::vector<uint8_t> makeCollidedIncrementalXrefPdf() {
  ...
  constexpr uint32_t filterSlots = (PdfLimits::PageRunBytes + PdfLimits::OperandOrderHistogramBytes) / sizeof(uint32_t);
  ...
  for (uint8_t revision = 0; revision < 3U; ++revision) {
    ...
    pdf += "trailer\n<< /Size " + std::to_string(size) + " /Root 1 0 R /Prev " + std::to_string(previousXref) +
...
std::vector<uint8_t> makeManyContentStreamsPdf(const uint8_t streamCount) {
  ...
  std::vector<std::string> objects = {
      "<< /Type /Catalog /Pages 2 0 R >>",
      "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
      "<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 " + std::to_string(fontObject) +
          " 0 R >> >> /Contents " + contents + " >>",
  };
  for (uint8_t index = 0; index < streamCount; ++index) {
    const std::string text = "Overflow" + (index < 10 ? std::string("0") : std::string()) + std::to_string(index);
...
std::vector<uint8_t> makeIndirectSeparationImagePdf() {
  const std::string content = "q 10 0 0 10 72 700 cm /Im1 Do Q BT /F1 12 Tf 72 650 Td (Readable page text) Tj ET";
  return assembleClassicPdf({
      "<< /Type /Catalog /Pages 2 0 R >>",
      "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
...
TEST(PdfPreparation, RestoresContentOverflowRecordsFromDiscoveryCheckpoint) {
  ...
  for (uint32_t slice = 0; slice < 20000 && interrupted.durableResumePhase() != PdfBuildResumePhase::AfterDiscovery;
       ++slice) {
  ...
  const std::string sectionPath =
      std::string(resumed.cacheRoot()) + "/gen_" + std::to_string(resumed.generation()) + "/sections/000000.xhtml";
  ASSERT_TRUE(harness.storage.exists(sectionPath));
  const auto& sectionBytes = harness.storage.bytes(sectionPath);
  const std::string section(sectionBytes.begin(), sectionBytes.end());
...
TEST(PdfPreparation, ManifestVerifierReopenStaysBoundedUnderSingleReaderContract) {
  ...
  for (uint32_t step = 0; step < 20000 && result.yielded(); ++step) {
    ...
    for (size_t index = opensBefore; index < opens.size(); ++index) {
      ...
      const bool manifestVerifier =
          observation.mode == PdfCacheOpenMode::Read &&
          (observation.path.ends_with("/manifest.a") || observation.path.ends_with("/manifest.b")) &&
...
TEST(PdfPreparation, FreshCacheSetupIoTraceRemainsStableAcrossRefactoring) {
  ...
  const std::string xrefPath =
      std::string(preparation.cacheRoot()) + "/gen_" + std::to_string(preparation.generation()) + "/build.xref.a";
  ...
  auto xrefOpen = std::find_if(wrongMode.begin(), wrongMode.end(),
                               [&](const PdfTestOpenObservation& observation) { return observation.path == xrefPath; });
...
TEST(PdfPreparation, ReflowsVisualLinesIntoHeadingsParagraphsAndDropCaps) {
  ...
  const std::vector<uint8_t>& bytes = harness.storage.bytes(sectionPath);
  const std::string xhtml(bytes.begin(), bytes.end());
  EXPECT_NE(xhtml.find("<h1 id=\"b00000000\">THE TWO-STEP PROCESS TO CHANGING YOUR IDENTITY</h1>"), std::string::npos)
      << xhtml;
  EXPECT_NE(xhtml.find("<h2 id=\"b00000001\">Why Tiny Changes Make a Big Difference</h2>"), std::string::npos) << xhtml;
...
TEST(PdfPreparation, KeepsTerminalPunctuationOnASecondHeadingLine) {
  ...
  PreparationHarness harness;
  harness.storage.addFile(sourcePath, makeTwoPageTextPdf(false, false, content.c_str(), "/Helvetica-Bold"), 1234, true);
  ...
  const std::vector<uint8_t>& bytes = harness.storage.bytes(sectionPath);
  const std::string xhtml(bytes.begin(), bytes.end());
  EXPECT_NE(xhtml.find(">HOW LONG DOES IT ACTUALLY TAKE TO FORM A NEW HABIT?</h2>"), std::string::npos) << xhtml;
...
TEST(PdfPreparation, RejoinsAWordSplitAtTheStartOfAHeadingFragment) {
  ...
  const std::vector<uint8_t>& bytes = harness.storage.bytes(sectionPath);
  const std::string xhtml(bytes.begin(), bytes.end());
  EXPECT_NE(xhtml.find(">THE MISMATCH BETWEEN IMMEDIATE AND DELAYED REWARDS</h2>"), std::string::npos) << xhtml;
}
...
TEST(PdfPreparation, DistinguishesStandaloneDropCapIFromIn) {
  ...
  for (const Case& testCase : cases) {
    ...
    const std::string content = std::string("BT ") + "/F1 32 Tf 1 0 0 1 72 540 Tm (I) Tj " +
    ...
    const std::string expected =
        std::string("<p id=\"b00000000\">") + testCase.expected + " This line completes the paragraph.</p>";
    EXPECT_NE(xhtml.find(expected), std::string::npos) << xhtml;
  }
}
...
TEST(PdfPreparation, KeepsSplitDropCapOpeningLineInTheParagraph) {
  ...
  const std::vector<uint8_t>& bytes = harness.storage.bytes(sectionPath);
  const std::string xhtml(bytes.begin(), bytes.end());
  EXPECT_NE(xhtml.find("<p id=\"b00000000\">HABITS CREATE THE FOUNDATION FOR MASTERY.</p>"), std::string::npos)
      << xhtml;
...
TEST(PdfPreparation, JoinsStackedChapterNumberAndTitleAtSectionStart) {
  ...
  const std::vector<uint8_t>& bytes = harness.storage.bytes(sectionPath);
  const std::string xhtml(bytes.begin(), bytes.end());
  EXPECT_NE(xhtml.find("<h1 id=\"b00000000\">1 The Surprising Power of Atomic Habits</h1>"), std::string::npos)
      << xhtml;
  EXPECT_EQ(xhtml.find("<h2"), std::string::npos) << xhtml;
}
...
TEST(PdfPreparation, OrdersColumnsDownBeforePreservingRowMajorTableCells) {
  ...
  PdfTestByteSource metadataSource(harness.storage.bytes(std::string(preparation.cacheRoot()) + "/gen_" +
                                                         std::to_string(preparation.generation()) + "/metadata.bin"));
  PdfMetadata metadata{};
  std::vector<PdfMetadataSection> sections;
  ASSERT_TRUE(pdfDecodeMetadata(metadataSource.source(), &metadata,
                                {&sections,
...
TEST(PdfPreparation, DoesNotTreatSparseInlineFragmentsAsASecondColumn) {
  ...
  for (uint8_t line = 0; line < 31U; ++line) {
    ...
    const int length = line < 6U ? std::snprintf(text, sizeof(text),
                                 ...
                                 : std::snprintf(text, sizeof(text),
                                                 "1 0 0 1 72 %u Tm (Ordinary line %u remains in the main text.) Tj ", y,
...
TEST(PdfPreparation, DoesNotTreatCenteredSingleColumnCreditsAsASecondColumn) {
  ...
  for (uint8_t line = 0; line < 6U; ++line) {
    char text[96]{};
    const int length = std::snprintf(text, sizeof(text), "1 0 0 1 250 %u Tm (Credit line %u.) Tj ",
                                     static_cast<unsigned>(740U - line * 16U), static_cast<unsigned>(line));
    ASSERT_GT(length, 0);
    ASSERT_LT(static_cast<size_t>(length), sizeof(text));
    content.append(text, static_cast<size_t>(length));
  }
...
TEST(PdfPreparation, DoesNotTreatStyledProseFragmentsAsWideTableCells) {
  ...
  PreparationHarness harness;
  harness.storage.addFile(sourcePath,
                          assembleClassicPdf({
                              "<< /Type /Catalog /Pages 2 0 R >>",
                              "<< /Type /Pages /Count 1 /Kids [3 0 R] >>",
...
TEST(PdfPreparation, SeparatesWideSameLineTableCellsWhenTheFontAlsoContainsSpaces) {
  ...
  for (uint8_t line = 0; line < 12; ++line) {
    char body[96]{};
    const int length =
        std::snprintf(body, sizeof(body), "1 0 0 1 72 %u Tm (Body line %u keeps prose in one column.) Tj ",
  ...
  const std::vector<uint8_t>& bytes = harness.storage.bytes(sectionPath);
  const std::string xhtml(bytes.begin(), bytes.end());
  EXPECT_NE(xhtml.find("<table><tbody><tr><td id=\"b00000001\">Very easy</td>"), std::string::npos) << xhtml;
  EXPECT_NE(xhtml.find(">Very hard</td></tr><tr><td id=\"b00000006\">Put on shoes</td>"), std::string::npos) << xhtml;
  EXPECT_NE(xhtml.find(">Run a marathon</td></tr><tr><td id=\"b0000000b\">Open your notes</td>"), std::string::npos)
      << xhtml;
  ...
  for (uint16_t row = 0; row < 86 && emitted < 256; ++row) {
    for (uint8_t column = 0; column < 3 && emitted < 256; ++column) {
      ...
      const int length = std::snprintf(
          line, sizeof(line), "1 0 0 1 %u %u Tm (%c%03u) Tj\n", static_cast<unsigned>(72U + column * 180U),
          static_cast<unsigned>(760U - row * 8U), static_cast<char>('A' + column), static_cast<unsigned>(row));
      ASSERT_GT(length, 0);
      ASSERT_LT(static_cast<size_t>(length), sizeof(line));
      content.append(line, static_cast<size_t>(length));
      ++emitted;
  ...
  for (const PdfTestFaultPoint faultPoint :
    ...
    for (const char* const failedLeaf : {"metadata.bin", "outline.bin"}) {
      ...
      const std::string failedPath =
          std::string(preparation.cacheRoot()) + "/gen_" + std::to_string(preparation.generation()) + "/" + failedLeaf;
...
TEST(PdfPreparation, RetainsPageResumeJournalBetweenCheckpoints) {
  ...
  const std::string journalPath =
      std::string(preparation.cacheRoot()) + "/gen_" + std::to_string(preparation.generation()) + "/resume.journal";
