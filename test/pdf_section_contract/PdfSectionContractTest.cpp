...
std::vector<uint8_t> makeOutlinedLinkPdf() {
  ...
  const std::string second = "BT /F1 12 Tf 72 720 Td (" + std::string(kNumericLinkText) +
                             ") Tj ET\n"
                             "BT /F1 12 Tf 72 660 Td (" +
                             std::string(kOutsideRectText) + ") Tj ET";
...
std::vector<uint8_t> makeNonOutlineTargetPdf() {
  ...
  const std::string second = "BT /F1 12 Tf 72 720 Td (" + std::string(kNonOutlineDestinationText) + ") Tj ET";
...
std::vector<uint8_t> makeRectEdgePdf() {
  const std::string first = "BT /F1 12 Tf 70 715 Td (" + std::string(kEdgeLinkText) + ") Tj ET\n" +
...
std::vector<uint8_t> makeRotatedCropPdf() {
  ...
  const std::string second = "BT /F1 12 Tf 72 720 Td (" + std::string(kRotatedDestinationText) + ") Tj ET";
  return makePdf({
      "<< /Type /Catalog /Pages 2 0 R /Outlines 8 0 R >>",
      "<< /Type /Pages /Count 2 /Kids [3 0 R 5 0 R] >>",
...
std::string manyOutlineTitle(const uint16_t index) {
  ...
  return length <= 0 || static_cast<size_t>(length) >= sizeof(title) ? std::string{}
...
std::vector<uint8_t> makeManyOutlinesPdf(const uint16_t count) {
  ...
  for (uint16_t index = 0; index < count; ++index) {
    ...
    objects.push_back("<< /Type /Page /Parent " + std::to_string(parentObject) +
                      " 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 " + std::to_string(fontObject) +
  ...
  for (uint16_t index = 0; index < count; ++index) {
    ...
    objects.push_back("<< /Title (" + manyOutlineTitle(index) + ") /Parent " + std::to_string(outlineRootObject) +
                      " 0 R" + links + " /Dest [" + std::to_string(pageObject) + " 0 R /XYZ null 720 null] >>");
  }
...
std::string longPageText(const uint16_t index) {
  ...
  return length <= 0 || static_cast<size_t>(length) >= sizeof(text) ? std::string{}
...
std::vector<uint8_t> makeLongNoOutlinePdf(const uint16_t count) {
  ...
  for (uint16_t index = 0; index < count; ++index) {
    ...
    objects.push_back("<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Resources << /Font << /F1 " +
                      std::to_string(fontObject) + " 0 R >> >> /Contents " + std::to_string(contentObject) + " 0 R >>");
...
std::string formatPublisherPageAnchor(const uint32_t sourcePageIndex) {
  char anchor[11]{};
  const int length = std::snprintf(anchor, sizeof(anchor), "p%08lx", static_cast<unsigned long>(sourcePageIndex));
...
bool outlineDefinesOnlySectionBoundary(const Observation& observation) {
  return observation.metadataDecoded && observation.metadata.sectionCount == 2U && observation.sections.size() == 2U &&
         ...
         observation.xhtml.size() == 2U && containsInOrder(observation.xhtml[0], kLeadText, kNumericLinkText) &&
...
bool preparedLinkLayoutDeclared(const std::string_view implementation) {
  ...
  return contains(compact, "structPreparedLink{") && contains(compact, "uint16_tsourcePage") &&
         ...
         contains(compact, "uint16_tyMin") && contains(compact, "uint16_txMax") && contains(compact, "uint16_tyMax") &&
         contains(compact, "static_assert(sizeof(PreparedLink)==12");
}
...
int runLayoutContract() {
  ...
  const std::string knownOldHeader = "struct PreparedPageRecord { uint32_t sourcePageIndex; uint32_t firstAnchor; };";
...
int runOracleControls() {
  ...
  report.check("CONTROL_PREFIX_AFTER_JOURNAL_TAIL_NEGATIVE",
               !prefixResumeSelectionMatches(PrefixCrashStage::AfterJournalBeforeCheckpoint, baselinePage, baselinePage,
                                             baselinePage + 1U, PdfBuildResumePhase::AfterPage),
  ...
  const std::string destinationXhtml = "<span id=\"" + publisherAnchor + "\" role=\"doc-pagebreak\"></span><p id=\"" +
  ...
  const Observation continuous = syntheticObservation({"<html><body><p>" + std::string(kFirstPageText) + "</p><p>" +
                                                       std::string(kSecondPageText) + "</p></body></html>"},
                                                      1);
  ...
  const Observation forcedPageSplit =
      syntheticObservation({"<html><body><p>" + std::string(kFirstPageText) + "</p></body></html>",
  ...
  const Observation reversedContinuous =
      syntheticObservation({"<html><body><p>" + std::string(kSecondPageText) + "</p><p>" + std::string(kFirstPageText) +
                            "</p></body></html>"},
                           1);
  ...
  const std::string outlinedFirst = "<p>" + std::string(kLeadText) + "</p><p><a href=\"" + pageDestinationHref + "\">" +
                                    std::string(kNumericLinkText) + "</a></p><p>" + std::string(kOutsideRectText) +
                                    "</p>";
  ...
  const Observation outlinedPerPage = syntheticObservation(
      {"<p>" + std::string(kLeadText) + "</p>",
       "<p>" + std::string(kNumericLinkText) + "</p><p>" + std::string(kOutsideRectText) + "</p>", destinationXhtml},
      3);
  ...
  const Observation reversedOutlined =
      syntheticObservation({"<p>" + std::string(kOutsideRectText) + "</p><p>" + std::string(kNumericLinkText) +
                                "</p><p>" + std::string(kLeadText) + "</p>",
                            destinationXhtml},
                           2);
  ...
  report.check("CONTROL_NUMERIC_LINK_POSITIVE",
               textLinkedToPreparedPageDestination(outlined, kNumericLinkText, kDestinationPageIndex, kDestinationText),
               "known-good numeric Rect text resolves to the target source-page anchor");
  const Observation missingNumericLink =
      syntheticObservation({"<p>" + std::string(kLeadText) + "</p><p>" + std::string(kNumericLinkText) + "</p><p>" +
  ...
  const Observation danglingNumericLink = syntheticObservation(
      {outlinedFirst, "<p id=\"" + std::string(kDestinationAnchor) + "\">" + std::string(kDestinationText) + "</p>"},
      2);
  ...
  const Observation overlinkedOutside = syntheticObservation(
      {"<p>" + std::string(kLeadText) + "</p><p>" + std::string(kNumericLinkText) + "</p><p><a href=\"" +
           pageDestinationHref + "\">" + std::string(kOutsideRectText) + "</a></p>",
       destinationXhtml},
      2);
  ...
  const Observation duplicatedOutside =
      syntheticObservation({"<p>" + std::string(kLeadText) + "</p><p>" + std::string(kNumericLinkText) + "</p><p>" +
...
int runNumericRectLinkContract() {
  ...
  report.check(
      "NUMERIC_RECT_TEXT_LINKED",
      textLinkedToPreparedPageDestination(observation, kNumericLinkText, kDestinationPageIndex, kDestinationText),
  ...
  std::cout << "OBSERVED outline_href=" << resolvedOutlineHref(observation)
            << " outline_prepared_href=" << preparedOutlineDestinationHref(observation) << " page_prepared_href="
            << preparedPageDestinationHref(observation, kDestinationPageIndex, kDestinationText) << " numeric_linked="
...
int runNonOutlineTargetContract() {
  ...
  report.check("NON_OUTLINE_TARGET_SECTION_CONTROL",
               observation.metadataDecoded && observation.metadata.sectionCount == 2U &&
  ...
  report.check("NON_OUTLINE_TARGET_LINKED",
               textLinkedToPreparedPageDestination(observation, kNonOutlineLinkText, 1U, kNonOutlineDestinationText),
  ...
  std::cout << "OBSERVED sections=" << observation.metadata.sectionCount
            << " target_href=" << preparedPageDestinationHref(observation, 1U, kNonOutlineDestinationText) << " linked="
            << textLinkedToPreparedPageDestination(observation, kNonOutlineLinkText, 1U, kNonOutlineDestinationText)
            << '\n';
  return report.failures == 0 ? 0 : 1;
}
...
int runRotatedCropContract() {
  ...
  std::cout << "OBSERVED rotated_href=" << preparedPageDestinationHref(observation, 1U, kRotatedDestinationText)
            << " linked="
            << textLinkedToPreparedPageDestination(observation, kRotatedLinkText, 1U, kRotatedDestinationText) << '\n';
  return report.failures == 0 ? 0 : 1;
}
...
int runOutlineCapacityContract() {
  ...
  std::cout << "OBSERVED completed=" << observation.completed
            << " error=" << static_cast<unsigned>(observation.terminalStatus.error)
            << " offset=" << observation.terminalStatus.offset << " outline=" << observation.outline.size()
  ...
  for (uint32_t step = 0; step < 1000000U; ++step) {
    ...
    if (resumed) {
      const bool beforeJournal = stage == PrefixCrashStage::AfterPrefixSyncBeforeJournal &&
                                 ...
                                 sectionZeroWasSynced(harness.storage);
      ...
      const bool afterCheckpoint = stage == PrefixCrashStage::AfterCheckpoint &&
                                   preparation.durableResumePage() >= baselinePage + PDF_CACHE_CHECKPOINT_PAGE_INTERVAL;
