...
TEST_F(LegacyStoreTest, PdfSpecificMutationsCannotModifyLoadedEpubStores) {
  ...
  ASSERT_EQ(clippings.addClipping(3, 1, 2, 4, 5, 8, 12, "Chapter", 7, "selected text", 0, 0),
...
TEST_F(LegacyStoreTest, PdfClippingPathMigrationIsAccepted) {
  {
    ...
    ASSERT_TRUE(source.loadForBook("/books/old.pdf", "Title", "Author", "pdf"));
    ASSERT_EQ(source.addClipping(1, 0, 0, 1, 0, 1, 2, "Chapter", 23, "text", 0, 0), ClippingStore::AddResult::Added);
  }
...
TEST_F(LegacyStoreTest, PdfDurableReloadReportsUntrustworthyReadForFailClosedReaderPath) {
  ...
  ASSERT_EQ(clippings.addPdfClipping(1, 0, 0, 1, 0, 0, 1, "Chapter", 8, "clipping"), ClippingStore::AddResult::Added);
  Storage.fail(TestStorageFault::Read);
  EXPECT_FALSE(clippings.reloadPdfFromDisk());
  Storage.clearFault();
...
struct StaleBookmarkLegacy {
  ...
  static PdfSavedItemsLegacyMutationResult clear(void* const context) {
    ...
    BookmarkStore durable;
    self.durableMutationApplied = durable.loadForBook(kPath, kTitle, kAuthor, "pdf") && durable.clearPdfBookmarks();
    return self.durableMutationApplied ? PdfSavedItemsLegacyMutationResult::Ambiguous
                                       : PdfSavedItemsLegacyMutationResult::Rejected;
  }
  ...
  PdfSavedItemsLegacyAccess callbacks() {
    return {this,
            StaleBookmarkLegacy::count,
            StaleBookmarkLegacy::read,
            StaleBookmarkLegacy::add,
            StaleBookmarkLegacy::remove,
            StaleBookmarkLegacy::clear};
  }
};
...
struct StaleClippingLegacy {
  ...
  static PdfSavedItemsLegacyMutationResult add(void* const context, const uint16_t itemId) {
    ...
    self.durableMutationApplied =
        durable.loadForBook(kPath, kTitle, kAuthor, "pdf") &&
        durable.addPdfClipping(1, 0, 0, 1, 0, 0, 1, "Chapter", itemId, "durable") == ClippingStore::AddResult::Added;
    return self.durableMutationApplied ? PdfSavedItemsLegacyMutationResult::Ambiguous
                                       : PdfSavedItemsLegacyMutationResult::Rejected;
  }
  ...
  static PdfSavedItemsLegacyMutationResult clear(void* const context) {
    ...
    if (self.operation != AmbiguousLegacyOperation::Clear) return PdfSavedItemsLegacyMutationResult::Rejected;
    ClippingStore durable;
    self.durableMutationApplied = durable.loadForBook(kPath, kTitle, kAuthor, "pdf") && durable.clearPdfClippings();
    return self.durableMutationApplied ? PdfSavedItemsLegacyMutationResult::Ambiguous
                                       : PdfSavedItemsLegacyMutationResult::Rejected;
  }
  ...
  PdfSavedItemsLegacyAccess callbacks() {
    return {this,
            StaleClippingLegacy::count,
            StaleClippingLegacy::read,
            StaleClippingLegacy::add,
            StaleClippingLegacy::remove,
            StaleClippingLegacy::clear};
  }
};
...
TEST(PdfSavedItemsSessionTest, DurableBookmarkFailAfterReloadsStaleResidentBeforeReconciliation) {
  for (const AmbiguousLegacyOperation operation :
    ...
    if (operation != AmbiguousLegacyOperation::Add) {
      ASSERT_EQ(legacy.resident.addPdfBookmark(1, 0.5F, "Chapter", 7, "resident"), BookmarkStore::AddResult::Added);
    ...
    const PdfSavedItemsSessionResult mutation =
        operation == AmbiguousLegacyOperation::Add
            ...
            : (operation == AmbiguousLegacyOperation::Remove ? session.remove(PdfSavedItemKind::Bookmark, 7)
  ...
  for (const AmbiguousLegacyOperation operation :
    ...
    const PdfSavedItemsSessionResult mutation =
        operation == AmbiguousLegacyOperation::Add
            ...
            : (operation == AmbiguousLegacyOperation::Remove ? session.remove(PdfSavedItemKind::Clipping, 7)
...
TEST(PdfSavedItemsSessionTest, DefinitiveLegacyRemoveAndClearRejectionNeverMutatesPsit) {
  for (const bool clearOperation : {false, true}) {
    ...
    const PdfSavedItemsSessionResult result = clearOperation ? fixture.session.clear(PdfSavedItemKind::Bookmark)
...
TEST(PdfSavedItemsSessionTest, DeleteCannotResurrectAfterPsitSaveFailureAndRebootReconcile) {
  for (const bool psitDeleteCommitted : {false, true}) {
    ...
    EXPECT_EQ(fixture.session.remove(PdfSavedItemKind::Bookmark, 7), PdfSavedItemsSessionResult::ReloadRequired);
    ASSERT_EQ(trace.count, 2);
...
TEST_F(LegacyStoreTest, BookmarkNoPathAllocationMatchesOwningCrcAndLegacyHashTrace) {
  ...
  const std::string_view exactPath(paddedPath, sizeof("/Books/legacy.epub") - 1U);
  const std::string ownedPath(exactPath);
  const uint32_t crc = uzlib_crc32(exactPath.data(), static_cast<unsigned int>(exactPath.size()), 0);
  const std::string currentPath = "/.crosspoint/bookmarks/epub_" + std::to_string(crc) + ".bin";
  const std::string legacyPath =
      "/.crosspoint/bookmarks/epub_" + std::to_string(std::hash<std::string>{}(ownedPath)) + ".bin";
  ASSERT_NE(currentPath, legacyPath);
  ...
  Storage.addFile(legacyPath, {2});
  Storage.beginBoundedPathCapture(64U);
  ASSERT_TRUE(BookmarkStore::deleteLegacyForFilePathNoPathAlloc(exactPath, "epub"));
...
TEST_F(LegacyStoreTest, BookmarkNoPathAllocationContinuesAfterCurrentRemoveFailure) {
  ...
  const std::string currentPath = "/.crosspoint/bookmarks/epub_" + std::to_string(crc) + ".bin";
  const std::string legacyPath =
      "/.crosspoint/bookmarks/epub_" + std::to_string(std::hash<std::string>{}(exactPath)) + ".bin";
  ASSERT_NE(currentPath, legacyPath);
  Storage.addFile(currentPath, {1});
  Storage.addFile(legacyPath, {2});
  Storage.fail(TestStorageFault::Remove, 1U);

  EXPECT_FALSE(BookmarkStore::deleteLegacyForFilePathNoPathAlloc(exactPath, "epub"));
...
TEST_F(LegacyStoreTest, ClippingNoPathAllocationMatchesOwningCrcTrace) {
  ...
  const std::string ownedPath(exactPath);
  const uint32_t crc = uzlib_crc32(exactPath.data(), static_cast<unsigned int>(exactPath.size()), 0);
  const std::string storePath = "/.crosspoint/clippings/epub_" + std::to_string(crc) + ".bin";
  ...
  Storage.addFile(storePath, {1});
  Storage.beginBoundedPathCapture(64U);
  ASSERT_TRUE(ClippingStore::deleteLegacyForFilePathNoPathAlloc(exactPath, "epub"));
...
TEST_F(LegacyStoreTest, PdfBookmarkNoPathAllocationPreservesTransactionalDeleteOrder) {
  constexpr char paddedPath[] = "/Books/delete.pdf.trailing";
  const std::string_view exactPath(paddedPath, sizeof("/Books/delete.pdf") - 1U);
  const std::string ownedPath(exactPath);
  const uint32_t crc = uzlib_crc32(exactPath.data(), static_cast<unsigned int>(exactPath.size()), 0);
  const std::string currentPath = "/.crosspoint/bookmarks/pdf_" + std::to_string(crc) + ".bin";
  const std::string legacyPath =
      "/.crosspoint/bookmarks/pdf_" + std::to_string(std::hash<std::string>{}(ownedPath)) + ".bin";
  ASSERT_NE(currentPath, legacyPath);
  const std::vector<std::string> paths = {currentPath + ".bak", currentPath + ".tmp", legacyPath + ".bak",
...
TEST_F(LegacyStoreTest, PdfBookmarkNoPathAllocationStopsBeforeAuthoritativePathsOnArtifactFailure) {
  const std::string exactPath = "/Books/delete.pdf";
  const uint32_t crc = uzlib_crc32(exactPath.data(), static_cast<unsigned int>(exactPath.size()), 0);
  const std::string currentPath = "/.crosspoint/bookmarks/pdf_" + std::to_string(crc) + ".bin";
  const std::string legacyPath =
      "/.crosspoint/bookmarks/pdf_" + std::to_string(std::hash<std::string>{}(exactPath)) + ".bin";
...
TEST_F(LegacyStoreTest, PdfClippingNoPathAllocationPreservesTransactionalDeleteOrder) {
  constexpr char paddedPath[] = "/Books/delete.pdf.trailing";
  const std::string_view exactPath(paddedPath, sizeof("/Books/delete.pdf") - 1U);
  const std::string ownedPath(exactPath);
  const uint32_t crc = uzlib_crc32(exactPath.data(), static_cast<unsigned int>(exactPath.size()), 0);
  const std::string canonicalPath = "/.crosspoint/clippings/pdf_" + std::to_string(crc) + ".bin";
  const std::vector<std::string> paths = {canonicalPath + ".bak", canonicalPath + ".tmp", canonicalPath};
  for (const std::string& path : paths) Storage.addFile(path, {1});
  Storage.beginBoundedPathCapture(68U);
...
TEST_F(LegacyStoreTest, NoPathAllocationStoreFormattersCheck64ByteBoundaryBeforeMutation) {
  ...
  const size_t bookmarkFixed = bookmarkPrefix.size() + std::max(currentSuffix.size(), legacySuffix.size());
  ASSERT_LT(bookmarkFixed, 63U);
  const std::string exactBookmarkType(63U - bookmarkFixed, 'b');
  const std::string currentBookmarkPath = bookmarkPrefix + exactBookmarkType + currentSuffix;
  const std::string legacyBookmarkPath = bookmarkPrefix + exactBookmarkType + legacySuffix;
  Storage.addFile(currentBookmarkPath, {1});
  Storage.addFile(legacyBookmarkPath, {2});
  Storage.beginBoundedPathCapture(64U);

  ASSERT_TRUE(BookmarkStore::deleteLegacyForFilePathNoPathAlloc(exactPath, exactBookmarkType));
  EXPECT_TRUE(Storage.boundedPathsWereNulTerminated());
  ...
  EXPECT_FALSE(BookmarkStore::deleteLegacyForFilePathNoPathAlloc(exactPath, overflowingBookmarkType));
  ...
  const std::string clippingPath = clippingPrefix + exactClippingType + currentSuffix;
  Storage.reset();
  Storage.addFile(clippingPath, {3});
  Storage.beginBoundedPathCapture(64U);
  ASSERT_TRUE(ClippingStore::deleteLegacyForFilePathNoPathAlloc(exactPath, exactClippingType));
  EXPECT_TRUE(Storage.boundedPathsWereNulTerminated());

  Storage.reset();
  Storage.addFile(clippingPath, {3});
  const std::string overflowingClippingType = exactClippingType + "x";
  EXPECT_FALSE(ClippingStore::deleteLegacyForFilePathNoPathAlloc(exactPath, overflowingClippingType));
