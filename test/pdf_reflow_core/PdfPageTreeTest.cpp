...
PdfStepResult walkBytes(std::vector<uint8_t> bytes, PageTreeHarness& harness,
  ...
  PdfPageTreeWalker walker(resolver, harness.arena, harness.traversalStorage.store(), capturePage, &harness,
                           setTraversalAccess, &harness, &harness.pageScratch,
                           harness.annotationOverflowStorage.store(), maxPages, harness.contentOverflowStorage.store());
...
TEST(PdfPageTreeTest, SpoolsContentStreamsBeyondInlineCapacityInSourceOrder) {
  ...
  for (uint16_t overflow = 0; overflow < page.overflowContentCount; ++overflow) {
    ...
    ASSERT_TRUE(pdfReadRecord(harness.contentOverflowStorage.store(), page.contentOverflowStart + overflow, &record));
...
TEST(PdfPageTreeTest, NormalizesAllQuarterTurnsAndIntersectsOrFallsBackFromCropBox) {
  PageTreeHarness harness;
...
TEST(PdfPageTreeTest, RejectsMalformedInheritedPageGeometry) {
  for (const char* fixture :
       {"page_tree_bad_media_box.pdf", "page_tree_bad_crop_box.pdf", "page_tree_bad_rotate.pdf"}) {
    PageTreeHarness harness;
...
TEST(PdfPageTreeTest, RejectsCycleAndDepth) {
  for (const auto& fixture : std::array<std::pair<const char*, PdfError>, 2>{
           {{"page_tree_cycle.pdf", PdfError::Malformed}, {"page_tree_deep.pdf", PdfError::LimitExceeded}}}) {
    PageTreeHarness harness;
