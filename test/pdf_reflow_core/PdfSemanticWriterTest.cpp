...
TEST(SemanticWriterTest, NormalizesUnsupportedPhoneticsAndNoBreakSpacesForDeviceFonts) {
  ...
  static constexpr uint8_t TEXT[] = {
      'w', 'o', 'r', 'd', 0xc2, 0xa0, 'w', 'r', 'a', 'p', ' ', 0xc9, 0x99, 0xcb, 0x88, 't', 0xc3, 0xa4, 'm', 'i', 'k',
  };
...
TEST(SemanticWriterTest, CollapsesWhitespaceAndOmitsSoftHyphensAcrossExtractionChunks) {
  ...
  ASSERT_TRUE(harness.writer.beginBlock({PdfSemanticBlockKind::Paragraph, 0, 0}).ok());
  ASSERT_TRUE(write(harness.writer, "  inter").ok());
  static constexpr uint8_t MIDDLE[] = {0xC2, 0xAD, 'n',  'a',  't', 'i', 'o', 'n', 'a',
                                       'l',  ' ',  '\t', '\n', 't', 'e', 'x', 't'};
...
TEST(SemanticWriterTest, RejoinsSplitCapitalLettersInsideHeadings) {
  ...
  EXPECT_NE(harness.output().find(">PROCESS TO CHANGE THAT QUICKLY HABITS MA ACTUALLY</h2>"), std::string::npos)
      << harness.output();
}
...
TEST(SemanticWriterTest, AddsMissingSpaceAfterNumberedHeadingPrefixAcrossChunks) {
  ...
  EXPECT_NE(harness.output().find(">3.6. Photocatalytic: Activity</h2>"), std::string::npos) << harness.output();
}
