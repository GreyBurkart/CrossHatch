...
struct TestResourceTable {
  ...
  static PdfStepResult finishInlineImage(void* context, const PdfByteSource& source, const uint64_t idEndOffset,
    ...
    if (!status.ok() || bytesRead != 1 ||
        (separator != ' ' && separator != '\t' && separator != '\r' && separator != '\n' && separator != '\f' &&
         separator != 0)) {
...
TEST(PdfContentInterpreterTest, UsesTjAdjustmentsInsteadOfFallbackWidthsForWordBoundaries) {
  ...
  PdfTestByteSource page(
      bytes("BT /F1 10 Tf 1 0 0 1 10 20 Tm "
...
TEST(PdfContentInterpreterTest, PreservesSoftHyphenWordJoinAfterRunWorkspaceFills) {
  ...
  EXPECT_EQ(std::string(reinterpret_cast<const char*>(model.text()), model.textLength()), std::string("degra\xC2\xAD"
                                                                                                      "dation"));
}
...
TEST(PdfContentInterpreterTest, KeepsSubscriptFragmentsInsideTheirVisualLine) {
  ...
  const auto appendRun = [&](const char* value, const uint16_t fontId, const int32_t xMin, const int32_t xMax,
...
TEST(PdfContentInterpreterTest, MergesBoldLabelsWithRegularTextWithoutMakingTheWholeLineBold) {
  ...
  const auto appendRun = [&](const char* value, const uint16_t fontId, const uint16_t flags, const int32_t xMin,
                             const int32_t xMax) {
    PdfTextRun run{};
    run.fontId = fontId;
...
TEST(PdfContentInterpreterTest, RetainsActualTextSoftHyphenAfterRunWorkspaceFills) {
  ...
  resources.font = &defaultFont.font;
  PdfTestByteSource page(
      bytes("BT /F1 10 Tf 1 0 0 1 10 20 Tm (degra) Tj "
  ...
  ASSERT_TRUE(result.complete()) << static_cast<int>(result.status.error);
  ASSERT_EQ(harness.model.runCount(), 1U);
  EXPECT_EQ(transcript(harness.model), std::string("degra\xC2\xAD"
                                                   "dation"));
}
...
TEST(PdfContentInterpreterTest, SeparatesRepeatedWordAcrossGeometricGapWhenTheFontContainsSpaces) {
  ...
  ASSERT_EQ(model.runCount(), 1U);
  EXPECT_EQ(std::string(reinterpret_cast<const char*>(model.text()), model.textLength()), "Continue Delay Delay time");
}
...
TEST(PdfContentInterpreterTest, AppliesFormBboxClipAndPageTransformToTextAndImages) {
  ...
  PdfTestByteSource form(
      bytes("BT /F1 10 Tf 1 0 0 1 20 30 Tm (Inside form) Tj "
...
TEST(PdfContentInterpreterTest, RejectsGraphicsOverflowButOmitsRecursiveForms) {
  ...
  EXPECT_NE(
      static_cast<uint16_t>(cycleHarness.model.warnings()) & static_cast<uint16_t>(PdfPageWarning::VectorArtOmitted),
      0U);
}
