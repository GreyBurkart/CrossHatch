...
struct LogicalXrefStore {
  ...
  static PdfStatus write(void*, uint32_t, const void*, size_t) { return PdfStatus::failure(PdfError::InvalidArgument); }
...
std::vector<uint8_t> loadXrefStreamFixture() {
  const std::filesystem::path path = std::filesystem::path(__FILE__).parent_path().parent_path() / "pdf_reflow_core" /
                                     "fixtures" / "xref_stream_objstm.pdf";
...
std::vector<uint8_t> oneObjectClassicPdf() {
  ...
  const int written =
      std::snprintf(xref, sizeof(xref),
                    "xref\n0 2\n0000000000 65535 f\n%010llu 00000 n\n"
                    "trailer\n<< /Size 2 /Root 1 0 R >>\nstartxref\n%llu\n%%%%EOF\n",
                    static_cast<unsigned long long>(objectOffset), static_cast<unsigned long long>(xrefOffset));
  EXPECT_GT(written, 0);
...
TEST(PdfXrefNewestObjectFilterContract, SegmentedBoundariesResetAndDetachStayWithinCallerOwnedSpans) {
  ...
  EXPECT_TRUE(std::all_of(tail.begin(), tail.begin() + kTailBytes, [](const uint8_t value) { return value == 0; }));
  EXPECT_EQ(tail.back(), 0xa5U);
  EXPECT_EQ(table.appendNewest({8'388'608U, 0, PdfXrefEntryType::Free, 0, 0, 0}).error, PdfError::InvalidArgument);
  ASSERT_TRUE(table.appendNewest(objectZero).ok());
  ASSERT_TRUE(table.appendNewest(objectMaximum).ok());
  EXPECT_EQ(table.entryCount(), 2U);
...
TEST(PdfMutableRecordSpoolContract, PartialHalBatchWriteErrorAbortClosesHandle) {
  ...
  EXPECT_EQ(spool.appendRecords(records.data(), static_cast<uint32_t>(records.size())).error, PdfError::IoFailure);
  EXPECT_FALSE(spool.isOpen());
  EXPECT_EQ(spool.recordCount(), 0U);
  EXPECT_EQ(storage.openHandleCount(), 0U);
}
...
TEST(PdfResolverSourceAccessContract, PageTreeReassertsXrefAfterTraversalEvictsDirectObjectStreamReader) {
  ...
  const std::string objectStream = "10 0 obj\n<< /Type /ObjStm /N 2 /First " + std::to_string(index.size()) +
