...
std::string fontObject() { return "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>"; }
...
std::vector<uint8_t> makeObjectStreamAnnotationHandoffPdf(const uint8_t annotationCount = 1U) {
  ...
  appendObject(xrefNumber, streamObjectWithDictionary(xref, "/Type /XRef /Size " + std::to_string(xrefNumber + 1U) +
                                                                " /Root 1 0 R /W [1 4 2] "));
  pdf += "startxref\n" + std::to_string(xrefOffset) + "\n%%EOF\n";
...
std::vector<uint8_t> makeIncrementalXrefPdf(const uint32_t entryCount = 71U) {
  ...
  output += "trailer\n<< /Size " + std::to_string(entryCount) + " /Root 1 0 R >>\nstartxref\n" +
  ...
  output += "trailer\n<< /Size " + std::to_string(entryCount) + " /Root 1 0 R /Prev " + std::to_string(baseXref) +
...
TEST(PdfDocumentSliceBudgetContract, SixtyFivePageRunBoundsEveryDiscoveryStepAndAvoidsRedundantTempSyncs) {
  ...
  for (uint32_t step = 0; step < 2'000'000U && result.yielded(); ++step) {
    ...
    terminalPhase = phase;
    const bool boundedPhase =
        phase == PdfPreparationPhase::ParseXref || phase == PdfPreparationPhase::SortXref ||
        phase == PdfPreparationPhase::ResolveCatalog || phase == PdfPreparationPhase::InitializePageTree ||
        phase == PdfPreparationPhase::WalkPages || phase == PdfPreparationPhase::FinalizePageTree ||
  ...
  EXPECT_LE(maximumMilliseconds, 5U) << "phase=" << static_cast<unsigned>(timePhase) << " opens=" << slowStep.opens
                                     << " reads=" << slowStep.reads << " writes=" << slowStep.writes
                                     << " closes=" << slowStep.closes << " readBytes=" << slowStep.bytesRead
                                     << " writeBytes=" << slowStep.bytesWritten << " trace=" << slowStepTrace;
  const std::string root = generationRoot(preparation);
...
TEST(PdfDocumentXrefFilterContract, CancelAfterCrossingSegmentBoundaryCanRestartSamePreparation) {
  ...
  for (uint32_t step = 0; step < 2'000'000U && preparation.phase() != PdfPreparationPhase::ParseXref; ++step) {
    const PdfStepResult result = preparation.step();
    ++harness.nowMs;
  ...
  for (uint32_t step = 0; step < 2'000'000U && preparation.phase() == PdfPreparationPhase::ParseXref &&
...
TEST(PdfDocumentResumeScaleContract, SameBatchDuplicateWithRecomputedIntegrityIsRejected) {
  ...
  const size_t recordsBytes =
      static_cast<size_t>(kXrefEntries) * kXrefRecordBytes + static_cast<size_t>(kPageCount) * kPageRecordBytes;
  ...
  writeLe32(journal, trailerOffset + 28U, pdfCacheCrc32(journal.data() + kHeaderBytes, recordsBytes));
  writeLe64(journal, trailerOffset + 32U, pdfCacheFnv64(journal.data() + kHeaderBytes, recordsBytes));
...
TEST(PdfDocumentResumeScaleContract, CancelledBatchedRestoreClosesHandlesAndPreservesPriorResumePoint) {
  ...
  for (uint32_t step = 0; step < 100'000U && !observedBatchWrite; ++step) {
    ...
    ++harness.nowMs;
    ASSERT_TRUE(result.yielded()) << static_cast<unsigned>(result.status.error) << "@" << result.status.offset;
    observedBatchWrite = std::find(harness.storage.operationTrace().begin(), harness.storage.operationTrace().end(),
                                   expectedWrite) != harness.storage.operationTrace().end();
  }
