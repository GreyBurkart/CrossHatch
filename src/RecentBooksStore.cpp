...
constexpr char RECENT_BOOKS_MOVE_TEMP[] = "/.crosspoint/recent.json.move.tmp";
constexpr char RECENT_BOOKS_MOVE_BACKUP[] = "/.crosspoint/recent.json.move.bak";
constexpr char RECENT_BOOKS_DELETE_TEMP[] = "/.crosspoint/recent.json.delete.tmp";
constexpr char RECENT_BOOKS_DELETE_BACKUP[] = "/.crosspoint/recent.json.delete.bak";
...
bool writeJsonBytes(void* const context, void* const fileContext) {
  ...
  return payload.data != nullptr &&
         file.write(reinterpret_cast<const uint8_t*>(payload.data), payload.length) == payload.length;
}
...
bool verifyJsonBytes(void* const context, const char* const path) {
  ...
  const auto& payload = *static_cast<const JsonBytes*>(context);
  FsFile file;
  if (payload.data == nullptr || !Storage.openFileForRead("RBS", path, file)) {
    return false;
  }
  ...
  while (offset < payload.length) {
    const size_t chunk = std::min(sizeof(actual), payload.length - offset);
    const int count = file.read(actual, chunk);
    if (count != static_cast<int>(chunk) || std::memcmp(actual, payload.data + offset, chunk) != 0) {
      matches = false;
      break;
    }
    offset += chunk;
...
bool cleanupRecentMoveArtifacts() {
  bool cleaned = true;
  if (Storage.exists(RECENT_BOOKS_MOVE_TEMP) && !Storage.remove(RECENT_BOOKS_MOVE_TEMP)) {
    cleaned = false;
  }
  if (Storage.exists(RECENT_BOOKS_MOVE_BACKUP) && !Storage.remove(RECENT_BOOKS_MOVE_BACKUP)) {
    cleaned = false;
  }
  return cleaned;
}
...
bool prepareRecentDeleteStateForRead() {
  if (BookMoveDurableFile::restoreCanonicalForRead(RecentBooksStore::getFilePath(), RECENT_BOOKS_DELETE_TEMP,
...
bool reserveRecentDeleteString(String& value, const size_t capacity) {
  ...
  if (recentDeleteFailReserveCall != 0 && recentDeleteReserveCalls == recentDeleteFailReserveCall) {
    return false;
  }
#endif
  ...
  return true;
#else
  return capacity <= std::numeric_limits<unsigned int>::max() && value.reserve(static_cast<unsigned int>(capacity));
#endif
}
...
[[gnu::noinline]] bool readStrictRecentDeleteDocument(JsonDocument& doc, String& json) {
  ...
  while (total < fileSize) {
    const size_t wanted = std::min(sizeof(chunk), static_cast<size_t>(fileSize - total));
    ...
    if (!json.concat(reinterpret_cast<const char*>(chunk), static_cast<unsigned int>(count)) ||
  ...
  const bool closed = file.close();
  if (!readComplete || !closed || total != fileSize || json.length() != fileSize) {
    return false;
  }
...
class RecentDeleteJsonWriter {
 public:
  explicit RecentDeleteJsonWriter(String& destination) : destination_(destination) {}
  ...
  size_t write(const uint8_t* const bytes, const size_t length) {
    ...
    if (destination_.length() >= recentDeleteSerializedByteLimit) {
      ...
      permitted = std::min(permitted, recentDeleteSerializedByteLimit - destination_.length());
    }
#endif
    const size_t before = destination_.length();
    if (permitted == 0 ||
        !destination_.concat(reinterpret_cast<const char*>(bytes), static_cast<unsigned int>(permitted)) ||
...
[[gnu::noinline]] bool serializeRecentDeleteDocument(const JsonDocument& doc, String& json) {
  ...
  return writer.complete() && !doc.overflowed() && written == measured && json.length() == measured;
}
}  // namespace
...
void failReserveOnCall(const size_t call) { recentDeleteFailReserveCall = call; }

void limitSerializedBytes(const size_t length) { recentDeleteSerializedByteLimit = length; }
...
bool RecentBooksStore::removeByPathDurably(const std::string& path) { return removeByPathDurablyNoPathAlloc(path); }

bool RecentBooksStore::removeByPathDurablyNoPathAlloc(const std::string_view path) {
  // Boot defers the recents load; migrations below edit the resident list.
  ensureLoaded();
  ...
  for (size_t index = books.size(); index > 0; --index) {
    ...
    if (path.size() == candidateLength && std::memcmp(path.data(), candidate, candidateLength) == 0) {
  ...
  if (serializeRecentDeleteDocument(doc, scratch->json)) {
    ...
    persisted =
        BookMoveDurableFile::replace(getFilePath(), RECENT_BOOKS_DELETE_TEMP, RECENT_BOOKS_DELETE_BACKUP, payload);
  } else {
    LOG_ERR("RBS", "Failed to serialize recent-book deletion state");
  }
  if (!persisted) return false;

  const auto found = std::find_if(recentBooks.begin(), recentBooks.end(), [&](const RecentBook& book) {
    return book.path.size() == path.size() && std::memcmp(book.path.data(), path.data(), path.size()) == 0;
  });
...
bool RecentBooksStore::activatePathMigration(const std::string& oldPath, const std::string& newPath,
                                             const std::string& oldCachePath, const std::string& newCachePath,
  ...
  auto oldEntry = std::find_if(recentBooks.begin(), recentBooks.end(),
                               [&](const RecentBook& book) { return book.path == oldPath; });
  auto newEntry = std::find_if(recentBooks.begin(), recentBooks.end(),
  ...
  auto rollbackRemoved = [&]() {
    for (int index = 1; index >= 0; --index) {
      ...
      recentBooks.insert(recentBooks.begin() + static_cast<std::ptrdiff_t>(removed.index), std::move(removed.book));
    }
  };
  ...
  } else if (oldEntry != recentBooks.end() && newEntry != recentBooks.end()) {
    removeInto(oldPath, scratch->removed[0]);
  ...
  if (serializeJson(doc, scratch->json) != 0) {
    ...
    const BookMoveDurableFile::Payload payload{&bytes, &writeJsonBytes, &verifyJsonBytes};
    Storage.mkdir("/.crosspoint");
    persisted = BookMoveDurableFile::replace(getFilePath(), RECENT_BOOKS_MOVE_TEMP, RECENT_BOOKS_MOVE_BACKUP, payload);
  } else {
  ...
  if (pathChanged) {
    ...
    if (oldEntry != recentBooks.end()) {
      ...
      if (scratch->coverChanged && oldEntry->coverBmpPath.rfind(newCachePath, 0) == 0) {
        oldEntry->coverBmpPath.replace(0, newCachePath.size(), oldCachePath);
      }
    }
  }
  rollbackRemoved();
...
bool RecentBooksStore::verifyPathMigration(const std::string& oldPath, const std::string& newPath,
  ...
  const size_t newCount = static_cast<size_t>(std::count_if(
      recentBooks.begin(), recentBooks.end(), [&](const RecentBook& book) { return book.path == newPath; }));
  return keepInRecents ? newCount <= 1 : newCount == 0;
}

bool RecentBooksStore::verifyPersistedPathMigration(const std::string& oldPath, const std::string& newPath,
                                                    const bool keepInRecents) const {
  ...
  return oldCount == 0 && (keepInRecents ? newCount <= 1 : newCount == 0);
}
...
bool RecentBooksStore::loadFromFile() {
  ...
  if (!Storage.exists(getFilePath())) {
    ...
    } else if (Storage.exists(RECENT_BOOKS_MOVE_TEMP) && !Storage.rename(RECENT_BOOKS_MOVE_TEMP, getFilePath())) {
  ...
  if (hasStoreFile && Storage.exists(RECENT_BOOKS_MOVE_BACKUP)) {
    if (!Storage.remove(getFilePath()) || !Storage.rename(RECENT_BOOKS_MOVE_BACKUP, getFilePath()) ||
