...
ClippingStore::AddResult ClippingStore::addClipping(const uint16_t spineIndex, const uint16_t startPage,
  ...
  if (isPdfStorePath(storeFilePath)) {
    // PDF stores keep text inline and commit through the PDF transaction; the
    // EPUB writer below would emit an offset-based layout the PDF reader rejects.
...
bool ClippingStore::readClippingPreview(const size_t index, std::string& out) const {
  ...
  if (isPdfStorePath(storeFilePath)) {
    // PDF clipping text stays resident (see Clipping::text); feed it through
    // the same bounded preview formatter as the SD-backed path.
    struct StringReader {
      const std::string& text;
      size_t offset = 0;
      int read(char* dst, const size_t len) {
        const size_t n = std::min(len, text.size() - offset);
        memcpy(dst, text.data() + offset, n);
        offset += n;
        return static_cast<int>(n);
      }
    } reader{clipping->text};
    return clipping->text.empty() || clippingPreview::read(reader, clipping->text.size(), out);
  }
...
bool ClippingStore::saveToFile() {
  ...
  // PDF stores keep text inline and are only rewritten transactionally.
  const bool written = isPdfStorePath(storeFilePath)
                           ? writePdfTransaction(nullptr, std::numeric_limits<size_t>::max(), false)
                           : writeToFile();
  if (written) {
    dirty = false;
    return true;
...
bool ClippingStore::writeMigrationPayload(void* const fileContext) const {
  ...
  for (uint16_t index = 0; wrote && index < count; ++index) {
    ...
    wrote =
        ...
        serialization::tryWritePod(file, clipping.layoutSignature) &&
        serialization::tryWritePod(file, clipping.tableSelection) &&
...
bool ClippingStore::verifyPdfTransaction(const std::string& path, const Clipping* const appended,
  ...
  for (size_t index = 0; matches && index < logicalCount; ++index) {
    ...
    matches = clipping != nullptr && readExpectedPod(file, clipping->spineIndex) &&
              ...
              readExpectedPod(file, clipping->layoutSignature) && readExpectedPod(file, clipping->tableSelection) &&
