...
struct JournalReadSession {
  ...
  JournalReadSession() : journal(makeJournalIo(workspace), {workspace.scratch, sizeof(workspace.scratch)}) {}

  static Io makeJournalIo(JournalIoWorkspace& workspace);
};

static_assert(sizeof(JournalReadSession) <= 7 * 1024, "PDF delete journal reader exceeded its RAM budget");
...
bool formatStorePath(const char* const directory, const char* const source, const size_t sourceLength,
  ...
  const int written =
      std::snprintf(destination, capacity, "%s/pdf_%lu.bin", directory, static_cast<unsigned long>(crc));
...
bool exactTargets(const DeleteWorkspace& workspace, const Targets& targets) {
  return sameView(targets.source, workspace.source) && sameView(targets.tombstone, workspace.tombstone) &&
         ...
         sameView(targets.clippings, workspace.clippings) && sameView(targets.recent, RecentBooksStore::getFilePath());
}
...
Status purgeBookmarks(void* const context, const Targets& targets) {
  const auto& workspace = *static_cast<const DeleteWorkspace*>(context);
  if (!exactTargets(workspace, targets)) return Status::InvalidArgument;
  return BookmarkStore::deletePdfForFilePathNoPathAlloc(std::string_view(workspace.source, targets.source.length))
             ? Status::Ok
...
Status purgeClippings(void* const context, const Targets& targets) {
  const auto& workspace = *static_cast<const DeleteWorkspace*>(context);
  if (!exactTargets(workspace, targets)) return Status::InvalidArgument;
  return ClippingStore::deletePdfForFilePathNoPathAlloc(std::string_view(workspace.source, targets.source.length))
             ? Status::Ok
             : Status::OperationFailed;
}
...
Status purgeRecents(void* const context, const Targets& targets) {
  const auto& workspace = *static_cast<const DeleteWorkspace*>(context);
  if (!exactTargets(workspace, targets)) return Status::InvalidArgument;
  return RECENT_BOOKS.removeByPathDurablyNoPathAlloc(std::string_view(workspace.source, targets.source.length))
             ? Status::Ok
             : Status::OperationFailed;
}
...
DeleteSession::DeleteSession()
    : journal(makeJournalIo(workspace.journalIo), {workspace.journalIo.scratch, sizeof(workspace.journalIo.scratch)}),
...
bool pathMatches(const StringView view, const std::string_view path) {
  return view.data != nullptr && view.length == path.size() && std::memcmp(view.data, path.data(), view.length) == 0;
}
...
void publishResult(const PdfDeleteUtils::Result result) {
  const bool absent = result == PdfDeleteUtils::Result::Complete || result == PdfDeleteUtils::Result::NoPendingDelete ||
...
PdfDeleteUtils::Result checkMoveFence(const std::string_view sourcePath) {
  const BookMutationFence fence = BookMoveUtils::mutationFenceForPathNoPathAlloc(sourcePath);
  if (fence == BookMutationFence::Clear) return PdfDeleteUtils::Result::NoPendingDelete;
  return fence == BookMutationFence::Indeterminate ? PdfDeleteUtils::Result::Pending : PdfDeleteUtils::Result::Conflict;
}
...
PdfDeleteUtils::Result recoverSession(DeleteSession& session, Selection selection) {
  ...
  const std::string_view sourcePath(session.workspace.source, selection.record.targets.source.length);
...
PdfDeleteUtils::Result deleteWithSession(DeleteSession& session, const std::string_view sourcePath) {
...
void DirectoryDeleteSessionDeleter::operator()(DirectoryDeleteSession* const session) const { delete session; }
...
Result deletePdfBookNoPathAlloc(DirectoryDeleteSession& session, const std::string_view sourcePath) {
  return deleteWithSession(session.session, sourcePath);
}
...
void markDeleteStartingForTest() { journalPresence.store(JournalPresence::DeleteStarting, std::memory_order_release); }
#endif
