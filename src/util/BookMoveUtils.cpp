...
MoveResult recoverPendingBookMove() {
  ...
  journalPresence.store(
      result == MoveResult::Complete || result == MoveResult::NoPendingMove || result == MoveResult::Abandoned
          ? JournalPresence::KnownAbsent
          : JournalPresence::LookupRequired,
      std::memory_order_release);
  return result;
...
BookMutationFence mutationFenceForPathNoPathAlloc(const std::string_view bookPath) {
  const JournalPresence presence = journalPresence.load(std::memory_order_acquire);
...
bool migrateMovedEpubState(const std::string& oldPath, const std::string& newPath, const std::string& oldCachePath,
  ...
  if (APP_STATE.pinnedBookPath == oldPath) {
    APP_STATE.pinnedBookPath = newPath;
    stateChanged = true;
  }
  if (stateChanged) {
