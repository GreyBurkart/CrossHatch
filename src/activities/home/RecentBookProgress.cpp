...
bool RecentBookProgress::PdfProductCache::load(const std::string& sourcePath) {
  ...
  if (!pdfFormatCacheRootForHash(PDF_CACHE_DIRECTORY, resolvedCacheHash, impl_->cacheRoot, sizeof(impl_->cacheRoot))) {
    impl_->cacheRoot[0] = '\0';
...
uint32_t RecentBookProgress::PdfProductCache::currentWord() const { return available() ? impl_->state.currentWord : 0; }

uint32_t RecentBookProgress::PdfProductCache::totalWords() const { return available() ? impl_->state.totalWords : 0; }
