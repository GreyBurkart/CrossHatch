...
bool deleteDirectoryPdf(void* const context, const char* const path) {
  ...
  auto* const deleteContext = static_cast<DirectoryDeleteContext*>(context);
  return deleteContext != nullptr && deleteContext->pdfSession && path != nullptr &&
         BookActions::deleteDirectoryPdfBookNoPathAlloc(*deleteContext->pdfSession, path);
#else
  (void)context;
  (void)path;
  return false;
...
bool prepareDirectoryPdfDelete(void* const context) {
  ...
  const PdfDeleteUtils::Result recovery = PdfDeleteUtils::recoverPendingPdfDelete();
  if (recovery != PdfDeleteUtils::Result::Complete && recovery != PdfDeleteUtils::Result::NoPendingDelete) {
    return false;
  }
  deleteContext->pdfSession = PdfDeleteUtils::makeDirectoryDeleteSessionNoThrow();
...
void FileBrowserActivity::promptDeleteDirectory(const std::string& fullPath, const std::string& entry,
  ...
  auto handler = [this, dirPath](const ActivityResult& res) {
    ...
    if (scanStatus == DirectoryMetadataScanStatus::PdfFound) {
      ...
      const PdfDirectoryDeleteScan::DeleteCallbacks callbacks{&deleteContext, &deleteDirectoryPdf,
                                                              &clearDirectoryLegacyMetadata, nullptr};
      ...
      if (status == PdfDirectoryDeleteScan::Status::CommittedWithCleanupWarning) {
        ...
        LOG_ERR("FileBrowser", "Directory delete failed for %s: %u", dirPath.c_str(), static_cast<unsigned>(status));
        return;
      }
