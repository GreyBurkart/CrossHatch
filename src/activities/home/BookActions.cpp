...
[[gnu::noinline]] void loadPdfRecentMetadata(const std::string& path, const std::string& displayName,
  ...
  const PdfCachedProductStateLoadResult loaded = pdfLoadCachedProductState(
      pdfHalCacheIo(workspace->io), path.c_str(), PDF_ACTION_CACHE_DIRECTORY, &workspace->state, cacheHashOverride);
...
[[gnu::noinline]] void updatePdfRecentAfterCompletion(const std::string& fullPath, const std::string& displayName,
...
void clearFileMetadata(const std::string& fullPath) {
  ...
  } else if (FsHelpers::hasPdfExtension(fullPath)) {
    // Drops every path-keyed PDF artifact. Used for a trashed PDF's original
    // path, which the crash-safe delete journal (keyed by the trash path) does
    // not own.
    char cacheRoot[64];
    const uint64_t cacheHash = pdfPathHash64(fullPath.data(), fullPath.size());
    if (pdfFormatCacheRootForHash("/.crosspoint", cacheHash, cacheRoot, sizeof(cacheRoot)).ok() &&
        Storage.exists(cacheRoot) && !Storage.removeDir(cacheRoot)) {
      LOG_ERR("BookActions", "Failed to remove PDF cache: %s", cacheRoot);
    }
    BookmarkStore::deleteForFilePath(fullPath, "pdf");
    ClippingStore::deleteForFilePath(fullPath, "pdf");
  }
}
...
bool clearDirectoryLegacyMetadataNoPathAlloc(const std::string_view fullPath) {
  bool success = true;
  if (FsHelpers::hasEpubExtension(fullPath)) {
    const bool cacheDeleted = Epub::clearCacheForFilePathNoPathAlloc(fullPath, "/.crosspoint");
    const bool bookmarksDeleted = BookmarkStore::deleteLegacyForFilePathNoPathAlloc(fullPath, "epub");
    const bool clippingsDeleted = ClippingStore::deleteLegacyForFilePathNoPathAlloc(fullPath, "epub");
    success = cacheDeleted && bookmarksDeleted && clippingsDeleted;
  } else if (FsHelpers::hasXtcExtension(fullPath)) {
    success = BookmarkStore::deleteLegacyForFilePathNoPathAlloc(fullPath, "xtc");
  } else if (FsHelpers::hasTxtExtension(fullPath) || FsHelpers::hasMarkdownExtension(fullPath)) {
    success = BookmarkStore::deleteLegacyForFilePathNoPathAlloc(fullPath, "txt");
  }
  LOG_DBG("BookActions", "Cleared directory metadata for: %.*s", static_cast<int>(fullPath.size()), fullPath.data());
  return success;
}
// PDF_BOOK_ACTIONS_PARITY_END: directory metadata

// PDF_BOOK_ACTIONS_PARITY_BEGIN: delete
bool deleteDirectoryPdfBookNoPathAlloc(PdfDeleteUtils::DirectoryDeleteSession& session,
                                       const std::string_view fullPath) {
  if (!FsHelpers::hasPdfExtension(fullPath)) return false;
  return PdfDeleteUtils::deletePdfBookNoPathAlloc(session, fullPath) == PdfDeleteUtils::Result::Complete;
}
...
bool deleteOrTrashFile(const std::string& fullPath, bool& movedToTrash) {
  if (SETTINGS.recycleBinEnabled && !crosshatch::trash::isPath(fullPath.c_str())) {
    ...
    // Journaled delete removes the file plus the cache, bookmarks, clippings
    // and recents keyed by fullPath, and resumes after a power loss.
    if (!deletePdfBook(fullPath)) {
      LOG_ERR("BookActions", "Failed to delete PDF safely: %s", fullPath.c_str());
      return false;
    }
    if (crosshatch::trash::isPath(fullPath.c_str())) {
      clearFileMetadata(fullPath.c_str() + strlen(crosshatch::trash::DIRECTORY));
    }
    movedToTrash = false;
  } else {
