...
struct HomeMenuEntries {
  // Continue Reading + Pinned Book + the nine entries appendHomeMenuItems can add.
...
bool HomeActivity::pinnedBookFitsOnHome(const bool hasContinueReadingRow) const {
  ...
  // Reading Stats is suppressed on this branch (updateHighlightedBookContext
  // always clears hasReadingStats), so it never takes a Home menu row.
  constexpr bool assumeReadingStats = false;
