...
bool isPdfUploadTarget(const String& filePath) { return FsHelpers::hasPdfExtension(filePath); }
    ...
    if (state.owner == UploadState::Owner::Http && state.file && state.error.isEmpty()) {
      ...
      while (remaining > 0) {
        ...
        if (state.bufferPos >= UploadState::UPLOAD_BUFFER_SIZE) {
          ...
          if (status != BookUpload::AtomicUploadStatus::Ok) {
            state.error =
                pdfUpload ? atomicUploadStatusMessage(status) : "Failed to write to SD card - disk may be full";
            if (!pdfUpload) {
              state.file.close();
            }
            return;
    ...
    if (state.owner == UploadState::Owner::Http) {
      ...
      if (state.error.isEmpty()) {
        ...
        if (writeStatus != BookUpload::AtomicUploadStatus::Ok) {
          state.error = pdfUpload ? atomicUploadStatusMessage(writeStatus) : "Failed to write final data to SD card";
        }
      }
...
void CrossPointWebServer::onWebSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_TEXT: {
      ...
      if (msg.startsWith("START:")) {
        ...
        if (firstColon > 0 && secondColon > 0) {
          ...
          const bool httpPdfActive = upload.owner == UploadState::Owner::Http && isPdfUploadTarget(upload.fileName);
          const bool sharedPdfActive = upload.owner == UploadState::Owner::WebSocket || httpPdfActive;
          if (wsUploadInProgress || sharedPdfActive || (pdfUpload && upload.owner != UploadState::Owner::None)) {
          ...
          if (!pdfUpload && Storage.exists(startValue.c_str())) {
            LOG_DBG("WS", "Upload collision: %s", startValue.c_str());
          ...
          if (wsUploadSize == 0) {
            if (pdfUpload) {
              ...
              ImageFolderIndex::invalidateForPath(startValue.c_str());
            }
    ...
    case WStype_BIN: {
      const bool pdfUpload = isPdfUploadTarget(wsUploadFileName);
      const bool uploadOpen = pdfUpload ? BookUpload::isActive(upload.transaction) : static_cast<bool>(wsUploadFile);
