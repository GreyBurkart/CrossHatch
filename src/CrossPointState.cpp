...
bool cleanupStateMoveArtifacts() {
  ...
  if (Storage.exists(STATE_MOVE_BACKUP) && !Storage.remove(STATE_MOVE_BACKUP)) {
    cleaned = false;
  }
  return cleaned;
}
...
bool CrossPointState::activateOpenPathMigration(const std::string& oldPath, const std::string& newPath) {
  ...
  const BookMoveDurableFile::Payload payload{this, &writeStatePayload, &verifyStatePayload};
  Storage.mkdir("/.crosspoint");
  if (BookMoveDurableFile::replace(STATE_FILE_JSON, STATE_MOVE_TEMP, STATE_MOVE_BACKUP, payload)) {
    return true;
  }
...
bool CrossPointState::verifyPersistedOpenPathMigration(const std::string& oldPath, const std::string& newPath) const {
...
bool CrossPointState::loadFromFile() {
  if (!Storage.exists(STATE_FILE_JSON)) {
    ...
    } else if (Storage.exists(STATE_MOVE_TEMP) && !Storage.rename(STATE_MOVE_TEMP, STATE_FILE_JSON)) {
  ...
  if (Storage.exists(STATE_FILE_JSON)) {
    ...
    if (Storage.exists(STATE_MOVE_BACKUP)) {
      if (!Storage.remove(STATE_FILE_JSON) || !Storage.rename(STATE_MOVE_BACKUP, STATE_FILE_JSON) ||
