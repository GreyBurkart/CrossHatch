...
class ReflowSectionSource {
 public:
  ...
  // Image helpers for sources that keep resources compressed (EPUB). Defaults
  // suit sources whose resources are already plain local files (PDF).
  virtual bool getOptimizerImageDimensions(const std::string&, uint16_t&, uint16_t&) const { return false; }
  // Streams only as much as `out` accepts; a short write ends the probe without error.
  virtual bool probeResource(const int sectionIndex, const std::string& href, Print& out,
                             const size_t chunkSize) const {
    return streamResource(sectionIndex, href, out, chunkSize);
  }
  virtual bool extractResourceToFile(int, const std::string&, const std::string&) const { return false; }
...
class ReflowDocument : public ReflowSectionSource {
 public:
  ...
  virtual std::string getCoverBmpPath(bool cropped = false, bool imageLevels = false) const = 0;
  virtual bool generateCoverBmp(bool cropped = false, const GfxRenderer* renderer = nullptr, int readerFontId = 0,
                                bool imageLevels = false) const = 0;
