...
bool formatEpubCachePath(char* const output, const size_t capacity, const char* const cacheDir, const uint64_t hash) {
  if (output == nullptr || capacity == 0U || cacheDir == nullptr) return false;
  const int written = snprintf(output, capacity, "%s/epub_%llu", cacheDir, static_cast<unsigned long long>(hash));
...
bool readLegacyProgressFile(const std::string& path, ReflowReadingPosition& position) {
  ...
  if (fileSize != 4 && fileSize != 6 && fileSize != 10) {
  ...
  decoded.sectionIndex = static_cast<int>(static_cast<uint16_t>(data[0]) | (static_cast<uint16_t>(data[1]) << 8));
  decoded.pageNumber = static_cast<int>(static_cast<uint16_t>(data[2]) | (static_cast<uint16_t>(data[3]) << 8));
  if (decoded.pageNumber == UINT16_MAX) decoded.pageNumber = 0;
  if (fileSize == 6 || fileSize == 10) {
    decoded.pageCount = static_cast<int>(static_cast<uint16_t>(data[4]) | (static_cast<uint16_t>(data[5]) << 8));
    decoded.hasPageCount = true;
  }
  if (fileSize == 10) {
    decoded.visibleTextOffset = static_cast<uint32_t>(data[6]) | (static_cast<uint32_t>(data[7]) << 8) |
                                (static_cast<uint32_t>(data[8]) << 16) | (static_cast<uint32_t>(data[9]) << 24);
    decoded.hasVisibleTextOffset = true;
  }
  position = decoded;
  return true;
}
...
bool Epub::clearCacheForFilePathNoPathAlloc(const std::string_view filepath, const char* const cacheDir) {
  if (cacheDir == nullptr) return false;

  char currentPath[64];
  char legacyPath[64];
  if (!formatEpubCachePath(currentPath, sizeof(currentPath), cacheDir,
                           ZipFile::fnvHash64(filepath.data(), filepath.size())) ||
      !formatEpubCachePath(legacyPath, sizeof(legacyPath), cacheDir, std::hash<std::string_view>{}(filepath))) {
    LOG_ERR("EBP", "EPUB cache path exceeds %u bytes", static_cast<unsigned>(sizeof(currentPath)));
    return false;
  }
  ...
  if (!Storage.exists(currentPath) && std::strcmp(legacyPath, currentPath) != 0 && Storage.exists(legacyPath)) {
    if (Storage.rename(legacyPath, currentPath)) {
      LOG_INF("EBP", "Migrated legacy EPUB cache: %s -> %s", legacyPath, currentPath);
    } else {
      migrationComplete = false;
      LOG_ERR("EBP", "Failed to migrate legacy EPUB cache: %s -> %s", legacyPath, currentPath);
    }
  }
  ...
  const bool legacyRemains = std::strcmp(legacyPath, currentPath) != 0 && Storage.exists(legacyPath);
  if (legacyRemains) {
...
bool Epub::formatCachePathForTest(char* const output, const size_t capacity, const char* const cacheDir,
                                  const uint64_t hash) {
...
bool Epub::probeResource(const int sectionIndex, const std::string& href, Print& out, const size_t chunkSize) const {
  if (sectionIndex < 0 || sectionIndex >= getSpineItemsCount()) {
    LOG_ERR("EBP", "Cannot probe resource for invalid section: %d", sectionIndex);
    return false;
  }
  return readItemContentsToStream(href, out, chunkSize, /*allowEarlyStop=*/true);
}

bool Epub::extractResourceToFile(const int sectionIndex, const std::string& href, const std::string& destPath) const {
  if (sectionIndex < 0 || sectionIndex >= getSpineItemsCount()) {
    LOG_ERR("EBP", "Cannot extract resource for invalid section: %d", sectionIndex);
    return false;
  }
...
ReflowSectionInfo Epub::getSectionInfo(const int sectionIndex) const {
  ...
  if (xLocationsLoaded && sectionIndex < static_cast<int>(locationSpineCount)) {
...
bool Epub::saveReadingPosition(const ReflowReadingPosition& position) const {
  ...
  const uint8_t data[10] = {
      ...
      static_cast<uint8_t>(position.visibleTextOffset & 0xFF),
      static_cast<uint8_t>((position.visibleTextOffset >> 8) & 0xFF),
      static_cast<uint8_t>((position.visibleTextOffset >> 16) & 0xFF),
      static_cast<uint8_t>((position.visibleTextOffset >> 24) & 0xFF),
  };
  const size_t dataSize = position.hasVisibleTextOffset ? sizeof(data) : 6;
  const size_t written = file.write(data, dataSize);
  if (written != dataSize) {
