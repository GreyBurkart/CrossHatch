...
struct PdfObjectResolverWorkspace {
  ...
  using CachedObjectStreamLookupFn = bool (*)(void* context, uint32_t objectStreamNumber, uint32_t* objectCount,
                                              uint64_t* first, uint64_t* size);
  using CachedObjectStreamPrepareFn = PdfStatus (*)(void* context, uint64_t requiredBytes, bool* cacheCleared);
  using CachedObjectStreamPublishFn = void (*)(void* context, uint32_t objectStreamNumber, uint32_t objectCount,
                                               uint64_t first, uint64_t size);
