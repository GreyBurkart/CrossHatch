...
bool recoverableCompletedCacheError(const PdfError error) {
  return error == PdfError::InvalidOffset || error == PdfError::InvalidArgument || error == PdfError::UnexpectedEof ||
         error == PdfError::Malformed || error == PdfError::LimitExceeded || error == PdfError::ExpansionLimit;
}
...
bool parseHex64(const char* const bytes, const size_t length, uint64_t* const value) {
  ...
  for (size_t index = 0; index < length; ++index) {
    ...
    const uint8_t nibble =
        bytes[index] <= '9' ? static_cast<uint8_t>(bytes[index] - '0') : static_cast<uint8_t>(bytes[index] - 'a' + 10);
...
bool parseImageLeaf(const char* const leaf, const size_t length, PdfCachedResourceRecord* const resource) {
  ...
  constexpr size_t jpegLength = hashDigits + 1U + crcDigits + 1U + lengthDigits + sizeof(jpegSuffix) - 1U;
  ...
  uint64_t nameCrc = 0;
  if (!parseHex64(leaf, hashDigits, &parsed.contentHash) || !parseHex64(leaf + hashDigits + 1U, crcDigits, &nameCrc)) {
    return false;
  }
  ...
  if (length == pixelLength) {
    ...
    if (leaf[commonEnd] != '-' || !parseHex64(leaf + commonEnd + 1U, lengthDigits, &parsed.encodedLength) ||
...
PdfStatus PdfReflowDocument::allocateValidationStorage(const uint16_t sectionCount, const uint8_t resourceCount) {
  ...
  if (!validationStorage_) {
    LOG_ERR("PDF", "Out of memory allocating %u-byte PDF validation storage", static_cast<unsigned>(layout.bytes));
...
PdfStatus PdfReflowDocument::classifyManifestRecord(ManifestRecordCursor* const cursor,
  ...
  *classification = {};
  ...
  const auto matchesArtifact = [&](const char* const leaf) {
    const int length =
        std::snprintf(expected, sizeof(expected), "gen_%lu/%s", static_cast<unsigned long>(cursor->generation), leaf);
...
PDF_REFLOW_DOCUMENT_NOINLINE PdfStatus PdfReflowDocument::validateCachedFile(const char* const path,
                                                                             const uint64_t expectedSize,
                                                                             const uint32_t expectedCrc32,
  ...
  if (resource->kind == PdfCachedResourceKind::Jpeg) {
    uint16_t width = 0;
    uint16_t height = 0;
    if (contentHash != resource->contentHash || crc != resource->nameCrc32 || expectedSize != resource->encodedLength ||
        !jpegDimensions.finish(&width, &height)) {
      return PdfStatus::failure(PdfError::Malformed);
    }
...
PDF_REFLOW_DOCUMENT_NOINLINE PdfStatus PdfReflowDocument::validateCapturedFiles() {
  ...
  for (uint16_t index = 0; index < manifestCursor_.sectionCount; ++index) {
    const int length = std::snprintf(validationPath_.data(), validationPath_.size(), "%s/gen_%lu/sections/%06u.xhtml",
    ...
    const PdfStatus status =
        validateCachedFile(validationPath_.data(), sectionRecords[index].size, sectionRecords[index].crc32, nullptr);
    if (!status) {
      return status;
    }
  }
  ...
  for (const Artifact& artifact : artifacts) {
    const int length =
        std::snprintf(validationPath_.data(), validationPath_.size(), "%s/gen_%lu/%s", cacheRoot_.c_str(),
                      static_cast<unsigned long>(manifestCursor_.generation), artifact.leaf);
    if (length <= 0 || static_cast<size_t>(length) >= validationPath_.size()) {
      return PdfStatus::failure(PdfError::LimitExceeded);
    }
    const PdfStatus status =
...
PDF_REFLOW_DOCUMENT_NOINLINE PdfStatus PdfReflowDocument::selectCompletedManifest(PdfCacheSlot* const selectedSlot,
                                                                                  const uint8_t excludedSlots,
                                                                                  bool* const candidateSelected) {
...
PDF_REFLOW_DOCUMENT_NOINLINE PdfStatus PdfReflowDocument::decodeSelectedManifest(const PdfCacheSlot selectedSlot) {
  ...
  const int pathLength =
      std::snprintf(validationPath_.data(), validationPath_.size(), "%s/%s", cacheRoot_.c_str(), manifestName);
  if (pathLength < 0 || static_cast<size_t>(pathLength) >= validationPath_.size()) {
    return PdfStatus::failure(PdfError::LimitExceeded);
  }
...
std::string PdfReflowDocument::getCoverBmpPath(bool, bool) const { return loaded_ ? coverPath_ : std::string{}; }
bool PdfReflowDocument::generateCoverBmp(bool, const GfxRenderer*, int, bool) const {
...
PdfStatus PdfReflowDocument::loadPdfSavedItems(PdfSavedItemsBuffer* const output) const {
  ...
  for (uint16_t index = 0; index < output->count; ++index) {
    ...
    uint16_t mappedSection = 0;
    if (!resolveWordSection(sections_.get(), metadata_.sectionCount, metadata_.totalWords, item.startGlobalWordOrdinal,
                            false, &mappedSection)) {
      output->count = 0;
...
PdfStatus PdfReflowDocument::mapPdfSavedItem(const std::string& sectionCachePath, const uint32_t layoutFingerprint,
                                             const PdfSavedItem& item, PdfSavedItemPageRange* const pages) const {
...
bool PdfReflowDocument::validateLayoutWordIndex(const std::string& sectionCachePath, const int sectionIndex,
  ...
  PdfStatus bindingStatus =
      io_.open(io_.context, sectionCachePath.c_str(), PdfCacheOpenMode::Read, &sectionSource.handle);
...
bool PdfReflowDocument::formatResourcePath(const PdfCachedResourceRecord& resource, char* const output,
  ...
  if (resource.kind == PdfCachedResourceKind::PixelCache) {
    written =
        std::snprintf(output, capacity, "%s/gen_%lu/images/%016llx-%08lx.pxc", cacheRoot_.c_str(),
                      static_cast<unsigned long>(generation), static_cast<unsigned long long>(resource.contentHash),
                      static_cast<unsigned long>(resource.nameCrc32));
  } else if (resource.kind == PdfCachedResourceKind::Jpeg) {
    written = std::snprintf(
        output, capacity, "%s/gen_%lu/images/%016llx-%08lx-%016llx.jpg", cacheRoot_.c_str(),
        static_cast<unsigned long>(generation), static_cast<unsigned long long>(resource.contentHash),
        static_cast<unsigned long>(resource.nameCrc32), static_cast<unsigned long long>(resource.encodedLength));
  }
  return written > 0 && static_cast<size_t>(written) < capacity;
}
...
void PdfReflowDocument::fitResourceDimensions(const uint16_t sourceWidth, const uint16_t sourceHeight,
  ...
  *width = static_cast<uint16_t>(std::max<uint64_t>(1U, (static_cast<uint64_t>(sourceWidth) * maximum) / largest));
  *height = static_cast<uint16_t>(std::max<uint64_t>(1U, (static_cast<uint64_t>(sourceHeight) * maximum) / largest));
}
...
bool PdfReflowDocument::resolveResource(const int sectionIndex, const std::string& href, ReflowResource& out) const {
  ...
  const ReflowImageKind kind =
      resource->kind == PdfCachedResourceKind::PixelCache ? ReflowImageKind::PixelCache : ReflowImageKind::EncodedImage;
