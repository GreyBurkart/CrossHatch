...
static_assert(sizeof(PdfPageTreeRecord) <= 72, "page-tree inheritance state must stay in fixed record storage");
static_assert(sizeof(PdfPageContentOverflowRecord) == 16, "content-overflow records must remain fixed-size");
static_assert(sizeof(PdfPageInfo) <= 312, "captured page metadata must stay bounded");
...
class PdfPageTreeWalker {
 public:
  ...
  PdfPageTreeWalker(PdfObjectResolver& resolver, PdfObjectArena& arena, PdfFixedRecordStore traversalStore,
                    ...
                    uint32_t maxPages = PdfLimits::MaxPages, PdfFixedRecordStore contentOverflowStore = {});
 ...
 private:
  ...
  PdfStepResult stepTraversalRecord(bool write, uint32_t ordinal, PdfPageTreeRecord* record, PdfWorkBudget& budget);
