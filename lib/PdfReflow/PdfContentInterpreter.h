...
struct PdfContentOperand {
  ...
  int64_t wideNumber() const {
    ...
    return bits <= static_cast<uint64_t>(INT64_MAX) ? static_cast<int64_t>(bits) : -1 - static_cast<int64_t>(~bits);
  }
};
...
class PdfContentInterpreter {
 ...
 private:
  ...
  static PdfStatus pageOverflowTextWrite(void* context, const uint8_t* source, size_t requested, size_t* bytesWritten);
