...
bool numericFixed(const PdfValue& value, int32_t* const result) {
  ...
  if (value.kind != PdfValueKind::Integer || value.integerValue < std::numeric_limits<int32_t>::min() / kFixedOne ||
      value.integerValue > std::numeric_limits<int32_t>::max() / kFixedOne) {
    return false;
  }
...
PdfStatus parsePageBox(const PdfObjectArena& arena, const uint16_t valueIndex, PdfRectangle* const result) {
  if (result == nullptr || valueIndex >= arena.valueCount || arena.values[valueIndex].kind != PdfValueKind::Array ||
  ...
  for (uint16_t ordinal = 0; ordinal < 4; ++ordinal) {
    ...
    if (!pdfArrayAt(arena, valueIndex, ordinal, &coordinateIndex) || coordinateIndex >= arena.valueCount ||
...
PdfStatus parsePageRotation(const PdfObjectArena& arena, const uint16_t valueIndex, uint16_t* const result) {
  if (result == nullptr || valueIndex >= arena.valueCount || arena.values[valueIndex].kind != PdfValueKind::Integer ||
...
PdfRectangle effectivePageBox(const PdfPageTreeRecord& inherited) {
  ...
  return intersection.xMin < intersection.xMax && intersection.yMin < intersection.yMax ? intersection
                                                                                        : inherited.mediaBox;
}

PdfStatus orientedPageExtent(const PdfPageTreeRecord& inherited, uint16_t* const width, uint16_t* const height) {
  ...
  const uint64_t fixedWidth = static_cast<uint64_t>(static_cast<int64_t>(box.xMax) - box.xMin);
  const uint64_t fixedHeight = static_cast<uint64_t>(static_cast<int64_t>(box.yMax) - box.yMin);
  const uint64_t roundedWidth = (fixedWidth + 65535U) >> 16U;
  const uint64_t roundedHeight = (fixedHeight + 65535U) >> 16U;
  if (roundedWidth == 0 || roundedHeight == 0 || roundedWidth > UINT16_MAX || roundedHeight > UINT16_MAX) {
...
PdfPageTreeWalker::PdfPageTreeWalker(PdfObjectResolver& resolver, PdfObjectArena& arena,
                                     ...
                                     const PdfFixedRecordStore annotationOverflowStore, const uint32_t maxPages,
...
PdfStepResult PdfPageTreeWalker::processResolvedNode(PdfWorkBudget& budget) {
  ...
  while (true) {
    ...
    if (processStage_ == ProcessStage::LoadAnnotation) {
      ...
      const PdfStatus status = pdfWriteRecord(annotationOverflowStore_, overflowAnnotationRecordCount_, &reference);
      if (!status) {
        return fail(status);
      }
    ...
    if (processStage_ == ProcessStage::EmitPage) {
      ...
      if (budget.stopRequested() || budget.operationsRemaining == 0 || budget.bytesRemaining < sizeof(PdfPageInfo)) {
        return PdfStepResult::paused();
      }
      --budget.operationsRemaining;
    ...
    if (processStage_ == ProcessStage::LoadContent) {
      if (contentsValueIndex_ >= arena_.valueCount || arena_.values[contentsValueIndex_].kind != PdfValueKind::Array) {
        return fail(PdfStatus::failure(PdfError::Malformed, current_.reference.objectNumber));
      }
      ...
      if (!pdfArrayAt(arena_, contentsValueIndex_, ordinal, &contentIndex) || contentIndex >= arena_.valueCount) {
        return fail(PdfStatus::failure(PdfError::Malformed, current_.reference.objectNumber));
      }
      ...
      constexpr uint16_t kMaximumOverflowContentCount = UINT16_MAX - PdfLimits::MaxContentStreamsPerPage;
      if (!contentOverflowStore_.valid() || contentOverflowStore_.recordSize != sizeof(PdfPageContentOverflowRecord) ||
    ...
    if (processStage_ == ProcessStage::BeginAnnotations) {
      processStage_ = annotationsValueIndex_ < arena_.valueCount &&
                              arena_.values[annotationsValueIndex_].kind == PdfValueKind::Array &&
