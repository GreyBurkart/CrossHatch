...
PdfStatus PdfSemanticWriter::flushPendingTextSpace(const uint8_t* const nextText, const size_t nextLength) {
  ...
  const bool ambiguousConjunction =
      nextText != nullptr && nextLength < 3U && (nextText[0] == 'a' || nextText[0] == 'o');
  const bool headingLetterContinuation =
      pendingTextSpace_ && !pendingTextHyphen_ && currentKind_ == PdfSemanticBlockKind::Heading &&
      lastTextByte_ >= 'A' && lastTextByte_ <= 'Z' && nextText != nullptr && nextLength == 1U && nextText[0] >= 'A' &&
      nextText[0] <= 'Z' &&
  ...
  const bool dehyphenate = pendingTextHyphen_ && pendingTextSpace_ && hyphenStemLength_ >= 3U && nextLowercase &&
                           !conjunction && !ambiguousConjunction;
...
PdfStatus PdfSemanticWriter::writeText(const uint8_t* const text, const size_t length) {
  ...
  while (offset < length) {
    ...
    const bool whitespace = scalar == ' ' || scalar == '\t' || scalar == '\r' || scalar == '\n' || scalar == 0x00A0U ||
                            (scalar >= 0x2000U && scalar <= 0x200AU) || scalar == 0x2028U || scalar == 0x2029U ||
