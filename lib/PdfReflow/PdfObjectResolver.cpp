...
PdfStepResult PdfObjectResolver::step(PdfWorkBudget& budget) {
  ...
  while (true) {
    ...
    if (phase_ == Phase::LookupReference) {
      ...
      if (lookupTargetReference().generation != 0 || xrefLookup_.entry.offset > PdfLimits::MaxIndirectObjectNumber) {
      ...
      if (!resolvingIndirectLength() && !hasResolvedIndirectLength() &&
        ...
        if (workspace_.lookupCachedObjectStream(workspace_.sourceAccessContext, objectStreamNumber_, &cachedCount,
                                                &cachedFirst, &cachedSize)) {
      ...
      phase_ =
          xrefLookup_.phase == PdfXrefLookupPhase::Done ? Phase::LookupObjectStream : Phase::SelectObjectStreamXref;
      return PdfStepResult::paused();
    }
    ...
    if (phase_ == Phase::SelectUncompressedSource || phase_ == Phase::SelectObjectStreamSource) {
      ...
      const PdfObjectReference reference = objectStream
                                               ? PdfObjectReference{objectStreamNumber_, xrefLookup_.entry.generation}
    ...
    switch (phase_) {
      ...
      case Phase::ObjectStreamIndexOffset: {
        ...
        const bool targetBoundaryKnown =
            objectStreamTargetFound_ && (objectStreamTargetIndex_ + 1U >= objectStreamObjectCount_ ||
...
PdfStatus PdfObjectResolver::beginIndirectLength(const PdfObjectReference reference) {
  ...
  phase_ = status.ok() ? (xrefLookup_.phase == PdfXrefLookupPhase::Done ? Phase::LookupReference : Phase::SelectXref)
                       : Phase::Failed;
  return status;
}
...
PdfStatus PdfObjectResolver::configureObjectStream(const uint64_t streamOffset) {
  ...
  if (workspace_.prepareCachedObjectStream != nullptr) {
    ...
    if (streamFilterCount_ != 0) {
      if (requiredBytes > workspace_.decodeLimits.maxExpandedBytes / workspace_.decodeLimits.maxExpansionRatio) {
    ...
    const PdfStatus prepareStatus =
        workspace_.prepareCachedObjectStream(workspace_.sourceAccessContext, requiredBytes, &cacheCleared);
    if (!prepareStatus) {
    ...
    if (!resolvingIndirectLength() && !hasResolvedIndirectLength()) {
      if (objectStreamUsesStore_ && workspace_.publishCachedObjectStream != nullptr) {
        workspace_.publishCachedObjectStream(workspace_.sourceAccessContext, objectStreamNumber_,
                                             objectStreamObjectCount_, objectStreamFirst_, objectStreamDecodedSize_);
      } else {
