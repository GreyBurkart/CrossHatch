...
struct PdfFixed16 {
  ...
  static constexpr PdfFixed16 fromInteger(const int16_t value) { return {static_cast<int32_t>(value) * 65536}; }
};
...
constexpr bool pdfFixedMultiply(const PdfFixed16 left, const PdfFixed16 right, PdfFixed16* result) {
  ...
  const int64_t scaled = product >= 0 ? (product + 32768) / 65536 : -(((-product) + 32768) / 65536);
...
constexpr bool pdfTransformPoint(const PdfMatrix& matrix, const PdfFixed16 x, const PdfFixed16 y,
  ...
  return transformedX != nullptr && transformedY != nullptr && pdfFixedMultiply(matrix.a, x, &ax) &&
         ...
         pdfFixedMultiply(matrix.d, y, &dy) && pdfFixedAdd(bx, dy, &sumY) && pdfFixedAdd(sumY, matrix.f, transformedY);
}
