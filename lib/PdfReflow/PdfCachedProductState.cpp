...
bool matchesRetainedImagePath(const PdfRequiredFileRecord& record, const char* const prefix) {
  ...
  const size_t jpegLength = prefixLength + commonLength + 1U + jpegFingerprintDigits + sizeof(jpegSuffix) - 1U;
  ...
  return std::memcmp(record.path + fingerprintOffset + jpegFingerprintDigits, jpegSuffix, sizeof(jpegSuffix) - 1U) == 0;
}
...
selectCompletedManifest(const PdfCacheIo& io, Workspace* const workspace, const PdfSourceIdentity* expectedSource,
...
PDF_CACHED_PRODUCT_NOINLINE PdfCachedProductStateLoadResult loadWithWorkspace(
    const PdfCacheIo& io, const char* const sourcePath, const char* const cacheDirectory,
    PdfCachedProductState* const productState, Workspace* const workspace, const uint64_t* const cacheHashOverride) {
  PdfStatus status =
...
PDF_CACHED_PRODUCT_NOINLINE PdfCachedProductStateLoadResult loadForSleepWithWorkspace(
    const PdfCacheIo& io, const char* const sourcePath, const char* const cacheDirectory,
    PdfCachedProductState* const productState, Workspace* const workspace, const uint64_t* const cacheHashOverride) {
  const PdfStatus status =
      cacheHashOverride == nullptr
...
#if defined(CROSSINK_ENABLE_PDF) && CROSSINK_ENABLE_PDF
PdfCachedProductStateLoadResult pdfLoadCachedProductStateForSleep(const PdfCacheIo& io, const char* const sourcePath,
                                                                  const char* const cacheDirectory,
                                                                  PdfCachedProductState* const productState,
                                                                  const uint64_t* const cacheHashOverride) {
  if (productState != nullptr) {
