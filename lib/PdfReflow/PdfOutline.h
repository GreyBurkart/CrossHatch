...
PdfStatus pdfReadNameTreeKid(const PdfObjectArena& arena, const PdfNameTreeNode& node, uint16_t ordinal,
                             PdfObjectReference* child);
PdfStatus pdfReadNameTreeLimits(const PdfObjectArena& arena, uint16_t rootIndex, char* first, size_t firstCapacity,
                                uint8_t* firstLength, char* last, size_t lastCapacity, uint8_t* lastLength);
...
PdfStatus pdfReadRawDestination(const PdfObjectArena& arena, uint16_t rootIndex, PdfRawDestination* destination);
