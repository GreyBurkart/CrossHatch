...
PdfStatus PdfXrefTable::flushPendingWrites() {
  ...
  if (!records_.valid() || records_.recordSize != sizeof(PdfXrefEntry) || appendBatchCount_ > entryCount_) {
...
bool PdfXrefTable::recordsAreTwoSortedRuns(uint32_t* const secondRunStart) const {
  if (secondRunStart == nullptr || appendOrderStrict_ || sortedRunCount_ != 2 || secondSortedRunStart_ == 0 ||
      secondSortedRunStart_ >= entryCount_) {
    return false;
  }
...
PdfStatus PdfXrefTable::beginFind(const uint32_t objectNumber, PdfXrefLookupState* const state) const {
  ...
  *state = {};
  ...
  } else if (!sampleIndexReady_ && !sampleIndexDisabled_ && lookupMissCount_ >= kSampleIndexLookupThreshold) {
...
PdfStepResult PdfXrefTable::stepFind(PdfXrefLookupState& state, PdfXrefEntry* const entry,
  ...
  while (true) {
    ...
    if (state.phase == PdfXrefLookupPhase::Verify) {
      ...
      lookupMissCount_ = std::min<uint8_t>(kSampleIndexLookupThreshold, static_cast<uint8_t>(lookupMissCount_ + 1U));
      rememberVictim(state.entry, state.first);
      const uint32_t ordinalDistance =
          hasLastLookupOrdinal_
              ? (state.first > lastLookupOrdinal_ ? state.first - lastLookupOrdinal_ : lastLookupOrdinal_ - state.first)
              : UINT32_MAX;
      const bool nearbyForward =
          hasLastLookupOrdinal_ && state.first > lastLookupOrdinal_ && ordinalDistance <= kLookupWindowEntries;
      if (nearbyForward) {
        localityStreak_ = std::min<uint8_t>(kLocalityStreakRequired, static_cast<uint8_t>(localityStreak_ + 1U));
      } else {
        localityStreak_ = 0;
      }
    ...
    if (state.phase == PdfXrefLookupPhase::ReadAhead) {
      ...
      --budget.operationsRemaining;
      budget.bytesRemaining -= sizeof(PdfXrefEntry);
      if (!pdfReadRecord(records_, lookupWindowFirstOrdinal_ + state.cursor, &lookupWindowEntries()[state.cursor])
               .ok()) {
        lookupWindowCount_ = 0;
        state.phase = PdfXrefLookupPhase::Done;
        *entry = state.entry;
...
PdfStatus PdfXrefTable::setSecurity(const PdfSecurityTrailer& security) {
  if (!security.encrypted || security.encryptionReference.objectNumber == 0 || security.fileIdentifierLength == 0 ||
  ...
  if (hasSecurity_) {
    return security_.encryptionReference == security.encryptionReference &&
                   ...
                   std::memcmp(security_.fileIdentifier, security.fileIdentifier, security.fileIdentifierLength) == 0
               ? PdfStatus::success()
  ...
  while (true) {
    ...
    switch (phase_) {
      case Phase::ExpectXref:
        ...
        if (!parseUnsigned(token, &streamObjectNumber_) || streamObjectNumber_ > PdfLimits::MaxIndirectObjectNumber) {
      ...
      case Phase::SectionCount: {
        ...
        uint64_t end = 0;
        if (!pdfCheckedAdd(subsectionStart_, value, &end) || end > PdfLimits::MaxIndirectObjectNumber + 1ULL) {
          return fail(PdfStatus::failure(PdfError::LimitExceeded, lexer_.tokenOffset()));
        }
        table_.prepareNewestObjectRange(subsectionStart_, static_cast<uint32_t>(value));
        const PdfStatus storageStatus = table_.preflightAppendRange(subsectionStart_, static_cast<uint32_t>(value));
        if (!storageStatus.ok()) {
...
PdfStatus PdfXrefParser::consumeCommonDictionary(const uint16_t rootIndex) {
  ...
  if (pdfDictionaryFind(trailerArena_, rootIndex, "Encrypt", &valueIndex)) {
    if (valueIndex >= trailerArena_.valueCount || trailerArena_.values[valueIndex].kind != PdfValueKind::Reference) {
    ...
    security.fileIdentifierLength = static_cast<uint8_t>(trailerArena_.values[firstIdentifierIndex].textLength);
    std::memcpy(security.fileIdentifier, trailerArena_.text + trailerArena_.values[firstIdentifierIndex].textOffset,
...
PdfStepResult PdfXrefParser::stepAdvanceXrefRange(PdfWorkBudget& budget) {
  while (true) {
    ...
    if (xrefIndexBufferPosition_ == xrefIndexBufferLength_) {
      if (xrefIndexReadOffset_ >= xrefIndexEndOffset_) {
        ...
        const PdfStatus status =
            source_.readAt(source_.context, xrefIndexReadOffset_, xrefIndexBuffer_, requested, &bytesRead);
        if (!status.ok()) {
          return PdfStepResult::failure(status);
        }
        if (bytesRead == 0 || bytesRead > requested) {
          return PdfStepResult::failure(
              PdfStatus::failure(bytesRead == 0 ? PdfError::UnexpectedEof : PdfError::IoFailure, xrefIndexReadOffset_));
        }
    ...
    uint64_t rangeEnd = 0;
    if (number == 0 || number > UINT32_MAX || !pdfCheckedAdd(xrefCurrentObject_, number, &rangeEnd) ||
