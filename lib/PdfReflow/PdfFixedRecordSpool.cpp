...
PdfStatus PdfFixedRecordSpool::configure(const PdfCacheIo* const io, const size_t recordSize, const uint32_t capacity) {
...
PdfStatus PdfFixedRecordSpool::open(const char* const path, const PdfCacheOpenMode mode,
  ...
  if (io_ == nullptr || handle_.valid() || path == nullptr || path[0] == '\0' || existingRecordCount > capacity_ ||
      mode == PdfCacheOpenMode::ReadWrite || (mode == PdfCacheOpenMode::WriteTruncate && existingRecordCount != 0)) {
...
PdfStatus PdfFixedRecordSpool::readRecords(const uint32_t ordinal, void* const records, const uint32_t count) {
  ...
  size_t bytesRead = 0;
  const PdfStatus status = io_->read(io_->context, handle_, offset, static_cast<uint8_t*>(records), bytes, &bytesRead);
  if (!status) {
    return status;
  }
  readOperations_ += count;
  return bytesRead == bytes ? PdfStatus::success() : PdfStatus::failure(PdfError::UnexpectedEof, offset + bytesRead);
}
...
PdfFixedRecordStore PdfMutableRecordSpool::store() { return {this, capacity_, recordSize_, readRecord, writeRecord}; }
