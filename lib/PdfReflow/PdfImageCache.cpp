...
PdfStatus PdfImageCache::beginJpegCapture(const uint8_t temporaryOrdinal, const uint64_t byteLimit,
  ...
  PdfStatus status =
      formatCapturedTemporaryPaths(temporaryOrdinal, fullPath, sizeof(fullPath), relativePath_, sizeof(relativePath_));
  if (!status.ok()) {
    return status;
  }
  status =
      pdfOpenTrackedCacheWriter(config_.io, fullPath, relativePath_, PdfCacheFileKind::Optional, byteLimit, &writer_);
  if (!status.ok()) {
    return status;
  }
  resetInPlace(*runtime);
...
PdfStatus PdfImageCache::beginCapturedJpeg(const PdfCapturedJpeg& captured, const uint16_t width, const uint16_t height,
                                           PdfCachedImage* const result, PdfImageCacheRuntime* const runtime) {
  if (!initialized_ || runtime == nullptr || runtime->stage != PdfImageCacheJpegStage::Idle || writer_.open ||
      writer_.fullPath[0] != '\0' || captured.sourceBytes == 0 || captured.temporaryOrdinal >= config_.entryCapacity ||
      width == 0 || height == 0 || result == nullptr) {
    return PdfStatus::failure(PdfError::InvalidArgument);
  }
  resetInPlace(*result);
  PdfStatus status = formatCapturedTemporaryPaths(captured.temporaryOrdinal, result->fullPath, sizeof(result->fullPath),
...
PdfStatus PdfImageCache::discardCapturedJpeg(const uint8_t temporaryOrdinal) {
  if (!initialized_ || writer_.open || writer_.fullPath[0] != '\0' || temporaryOrdinal >= config_.entryCapacity) {
    return PdfStatus::failure(PdfError::InvalidArgument);
  }
  char fullPath[PDF_CACHE_PATH_CAPACITY]{};
  char relativePath[PDF_CACHE_REQUIRED_PATH_CAPACITY]{};
  const PdfStatus status =
      formatCapturedTemporaryPaths(temporaryOrdinal, fullPath, sizeof(fullPath), relativePath, sizeof(relativePath));
  if (!status.ok()) {
    return status;
  }
