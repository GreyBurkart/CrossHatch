...
bool pdfGlyphNameToTextMapping(const uint8_t* const name, const size_t length, uint32_t* const mapping) {
  return pdfGlyphNameToUnicode(name, length, mapping) || packGlyphNameSequence(name, length, mapping);
}
...
PdfStatus PdfSimpleEncoding::begin(const PdfBaseEncoding base, const uint16_t existingDifferenceCount) {
