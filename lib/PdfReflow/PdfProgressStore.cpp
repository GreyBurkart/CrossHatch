...
PdfStatus PdfProgressStore::load(ReflowReadingPosition* const position) const {
  ...
  const PdfStatus aStatus =
      pendingSlot_ == PendingSlot::A ? PdfStatus::failure(PdfError::InvalidOffset) : readSlot(kSlotA, &a);
  const PdfStatus bStatus =
      pendingSlot_ == PendingSlot::B ? PdfStatus::failure(PdfError::InvalidOffset) : readSlot(kSlotB, &b);
