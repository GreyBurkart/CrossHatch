...
PdfStatus PdfJpegPreview::beginHeader(const PdfCacheIo& io, const PdfCacheHandle source, const uint64_t sourceBytes,
  ...
  reset();
  if (!io.valid() || !source.valid() || sourceBytes < 4U || ioBuffer == nullptr || ioBufferBytes < kEntropyHeadroom ||
      ioBufferBytes > PdfLimits::SourceBufferBytes || workspace == nullptr || workspaceBytes < 64U) {
...
PdfStepResult PdfJpegPreview::stepHeader(PdfWorkBudget& budget) {
  ...
  if (headerStage_ == HeaderStage::Marker) {
    ...
    const bool inspect = marker_ == 0xc0 || marker_ == 0xc1 || marker_ == 0xc2 || marker_ == 0xc3 || marker_ == 0xc4 ||
                         marker_ == 0xda || marker_ == 0xdb || marker_ == 0xdd;
    if (!inspect) {
...
PdfStatus PdfJpegPreview::parseScan(const uint8_t* const payload, const size_t length) {
  ...
  mcuColumns_ = static_cast<uint16_t>((static_cast<uint32_t>(width_) + 8U * maximumHorizontalSampling_ - 1U) /
...
PdfStatus PdfJpegPreview::beginAsset(PdfCacheTrackedWriter* const writer, const uint16_t targetWidth,
  ...
  if (widthLimitedHeight <= targetHeight) {
    ...
    scaledWidth_ = static_cast<uint16_t>(std::max<uint64_t>(1, static_cast<uint64_t>(width_) * targetHeight / height_));
  }
...
PdfStepResult PdfJpegPreview::refillEntropy(PdfWorkBudget& budget) {
  ...
  const size_t requested = static_cast<size_t>(std::min<uint64_t>(ioBufferBytes_, sourceBytes_ - offset));
...
PdfStatus PdfJpegPreview::decodeBlock(Component& component, const bool luminance, const uint8_t blockIndex) {
  ...
  if (luminance) {
    const int32_t dc = component.predictor * static_cast<int32_t>(quantizationDc_[component.quantizationTable]);
    ...
    const uint16_t blockX = static_cast<uint16_t>(currentMcuX_ * maximumHorizontalSampling_ + localX);
...
void PdfJpegPreview::advanceDecodedBlock() {
  ...
  currentMcuX_ = 0;
  ++currentMcuY_;
  if (restartInterval_ != 0 && mcusSinceRestart_ == restartInterval_ && currentMcuY_ < mcuRows_) {
    restartPending_ = true;
  }
}
...
bool PdfJpegPreview::currentDecodedRowCanServeOutput() const {
  if (!decodedRowReady_ || outputRow_ < offsetY_ || outputRow_ >= static_cast<uint16_t>(offsetY_ + scaledHeight_)) {
    return false;
  }
  const uint16_t sourceY =
      static_cast<uint16_t>(static_cast<uint32_t>(outputRow_ - offsetY_) * height_ / scaledHeight_);
...
PdfStepResult PdfJpegPreview::writeImageRow(PdfWorkBudget& budget) {
  ...
  for (uint16_t x = offsetX_; x < imageEndX; ++x) {
    const uint16_t sourceX = static_cast<uint16_t>(static_cast<uint32_t>(x - offsetX_) * width_ / scaledWidth_);
...
PdfStepResult PdfJpegPreview::stepAsset(PdfWorkBudget& budget) {
  ...
  if (assetStage_ == AssetStage::Decode) {
    ...
    const PdfStatus status = decodeBlock(component, componentIndex == luminanceComponent_, currentComponentBlock_);
    if (!status) {
      return PdfStepResult::failure(status);
    }
    advanceDecodedBlock();
