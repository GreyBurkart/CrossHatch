...
class PdfReflowDocument : public ReflowDocument {
 public:
  ...
  PdfStatus mapPdfSavedItem(const std::string& sectionCachePath, uint32_t layoutFingerprint, const PdfSavedItem& item,
                            PdfSavedItemPageRange* pages) const;
...
static_assert(sizeof(PdfReflowDocument) <= 2560, "PdfReflowDocument must not embed maximum-capacity validation tables");
