...
struct PdfWorkBudget {
  ...
  constexpr bool cancelRequested() const { return stopRequestedFn != nullptr && stopRequestedFn(stopContext); }

  constexpr bool stopRequested() const {
    return cancelRequested() || (yieldRequestedFn != nullptr && yieldRequestedFn(yieldContext));
  }
