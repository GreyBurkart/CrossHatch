...
bool checkedAdd64(const int64_t left, const int64_t right, int64_t* const result) {
  if (result == nullptr || (right > 0 && left > INT64_MAX - right) || (right < 0 && left < INT64_MIN - right)) {
    return false;
  }
  *result = left + right;
  return true;
...
bool checkedMultiply64(const int64_t left, const int64_t right, int64_t* const result) {
  ...
  if ((left > 0 && right > 0 && left > INT64_MAX / right) || (left > 0 && right < 0 && right < INT64_MIN / left) ||
      (left < 0 && right > 0 && left < INT64_MIN / right) || (left < 0 && right < 0 && left < INT64_MAX / right)) {
    return false;
  }
  *result = left * right;
  return true;
...
bool multiplyFixedWide(const int32_t left, const int64_t right, int64_t* const result) {
  ...
  const int64_t fractional =
      fractionalProduct >= 0 ? (fractionalProduct + 32768) / 65536 : -(((-fractionalProduct) + 32768) / 65536);
...
bool concatenateWide(const PdfMatrix& current, const PdfContentOperand* const next, PdfMatrix* const result) {
  ...
  const auto component = [&next](const int32_t leftFirst, const uint8_t rightFirst, const int32_t leftSecond,
                                 const uint8_t rightSecond, const int32_t add, PdfFixed16* const output) {
    int64_t first = 0;
    int64_t second = 0;
    int64_t sum = 0;
    if (output == nullptr || !multiplyFixedWide(leftFirst, next[rightFirst].wideNumber(), &first) ||
        !multiplyFixedWide(leftSecond, next[rightSecond].wideNumber(), &second) || !checkedAdd64(first, second, &sum) ||
        !checkedAdd64(sum, add, &sum) || sum < INT32_MIN || sum > INT32_MAX) {
      return false;
    }
...
bool isVectorOperator(const PdfToken& token) {
  ...
  return token.length == 2U &&
         ((token.bytes[0] == 'r' && token.bytes[1] == 'e') ||
          ((token.bytes[0] == 'f' || token.bytes[0] == 'B' || token.bytes[0] == 'b' || token.bytes[0] == 'W') &&
           token.bytes[1] == '*'));
}
...
PdfStatus PdfContentInterpreter::begin(const PdfByteSource* const contentSources, const uint8_t contentSourceCount,
  ...
  if (contentSources == nullptr || contentSourceCount == 0 ||
      ...
      workspace_.sourceBufferSize == 0 || workspace_.sourceBufferSize > PdfLimits::InterpreterSourceBufferBytes ||
...
PdfStepResult PdfContentInterpreter::step(PdfWorkBudget& budget) {
  ...
  return budget.cancelRequested() ? fail(PdfStatus::failure(PdfError::Cancelled, lexer_.position()))
...
PdfStatus PdfContentInterpreter::processOperator(const PdfToken& token) {
  ...
  if (tokenEquals(token, "q") || tokenEquals(token, "Q") || tokenEquals(token, "cm") || tokenEquals(token, "gs") ||
      tokenEquals(token, "g") || tokenEquals(token, "rg") || tokenEquals(token, "k") || tokenEquals(token, "sc") ||
      tokenEquals(token, "scn")) {
...
PdfStatus PdfContentInterpreter::processTextOperator(const PdfToken& token) {
  ...
  const bool textStateOperator = tokenEquals(token, "Tc") || tokenEquals(token, "Tw") || tokenEquals(token, "Tz") ||
                                 tokenEquals(token, "TL") || tokenEquals(token, "Ts") || tokenEquals(token, "Tr");
  ...
  if (tokenEquals(token, "TJ")) {
    ...
    if (workspace_.operands[0].reserved != 0) {
      return workspace_.operands[0].itemCount == 0 ? flushPendingArrayWhitespace() : failStatus(PdfError::Malformed);
    }
...
PdfStatus PdfContentInterpreter::transformedAxisAlignedRectangle(const PdfRectangle& rectangle,
  ...
  for (uint8_t corner = 0; corner < 4; ++corner) {
    const PdfStatus status =
        transformedGraphicsPoint(inputX[corner], inputY[corner], &outputX[corner], &outputY[corner]);
    if (!status.ok()) {
      return status;
    }
  }
...
PdfStatus PdfContentInterpreter::processPathOperator(const PdfToken& token) {
  ...
  if (tokenEquals(token, "re")) {
    ...
    PdfStatus status =
        transformedAxisAlignedRectangle({x.raw, y.raw, xEnd.raw, yEnd.raw}, &currentPathRectangle_, &axisAligned);
    if (!status.ok()) {
      return status;
    }
    if (!axisAligned) {
...
PdfStatus PdfContentInterpreter::processMarkedContentOperator(const PdfToken& token) {
  const auto startsBlock = [&](const PdfContentOperand& tag) {
    ...
    return equals("P", 1U) || equals("Figure", 6U) || equals("H", 1U) ||
           (tag.textLength == 2U && name[0] == 'H' && name[1] >= '1' && name[1] <= '6') || equals("L", 1U) ||
           equals("LI", 2U) || equals("LBody", 5U) || equals("BlockQuote", 10U) || equals("Caption", 7U);
  };
...
PdfStatus PdfContentInterpreter::makeRun(PdfTextRun* const run, const uint16_t flags) const {
  ...
  *run = {};
  ...
  if (arrayPendingAdjustment_ != INT32_MAX) {
    ...
    run->flags |=
        arrayPendingAdjustment_ <= WORD_GAP_ADJUSTMENT ? PdfTextArrayExplicitGap : PdfTextArrayTightContinuation;
  }
...
PdfStatus PdfContentInterpreter::emitActualText(MarkedContentFrame& frame) {
  ...
  frame.flags =
      static_cast<uint8_t>((frame.flags & ~kMarkedContentOverflow) | (overflow ? kMarkedContentOverflow : 0U));
...
PdfStatus PdfContentInterpreter::finishSemanticTextRun() {
  ...
  const bool intersectsPage =
      !workspace_.hasPageBounds || (run->xMax > workspace_.pageBounds.xMin && run->xMin < workspace_.pageBounds.xMax &&
                                    run->yMax > workspace_.pageBounds.yMin && run->yMin < workspace_.pageBounds.yMax);
  const bool intersectsClip =
      !graphics_.hasClip || (run->xMax > graphics_.clip.xMin && run->xMin < graphics_.clip.xMax &&
                             run->yMax > graphics_.clip.yMin && run->yMin < graphics_.clip.yMax);
    ...
    if (item.kind == PdfContentOperandKind::String) {
      ...
      if (decodedString && allWhitespace) {
        ...
        arrayPendingAdjustment_ =
            static_cast<int32_t>(std::max<int64_t>(INT32_MIN, std::min<int64_t>(INT32_MAX - 1LL, pending)));
        continue;
      }
      const PdfStatus pendingStatus = flushPendingArrayWhitespace();
      if (!pendingStatus) {
...
PdfStatus PdfContentInterpreter::appendImage(const PdfContentXObject& image, const bool inlineImage) {
  ...
  const bool intersectsClip =
      !graphics_.hasClip || (placement.xMax > graphics_.clip.xMin && placement.xMin < graphics_.clip.xMax &&
