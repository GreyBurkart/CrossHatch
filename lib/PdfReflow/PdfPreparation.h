...
class PdfPreparation {
 ...
 private:
  ...
  static PdfStatus readSource(void* context, uint64_t offset, uint8_t* destination, size_t requested,
                              size_t* bytesRead);
  static PdfStepResult setResolverSourceAccess(void* context, PdfObjectResolverReader reader, PdfWorkBudget& budget);
  ...
  static PdfStatus writeContentOverflowRecord(void* context, uint32_t ordinal, const void* record, size_t recordSize);
  ...
  static PdfStatus discardInlineImageDecoded(void* context, const uint8_t* source, size_t requested,
                                             size_t* bytesWritten);
  static PdfStatus readPreparedContentSource(void* context, uint64_t offset, uint8_t* destination, size_t requested,
                                             size_t* bytesRead);
  static PdfStatus readPreparedFormContent(void* context, uint64_t offset, uint8_t* destination, size_t requested,
                                           size_t* bytesRead);
  static PdfStepResult replayPreparedInlineImage(void* context, const PdfByteSource& source, uint64_t idEndOffset,
  ...
  static PdfStatus readPreparedContentStore(void* context, uint64_t offset, uint8_t* destination, size_t requested,
                                            size_t* bytesRead);
  ...
  static PdfStatus readResolverObjectStore(void* context, uint64_t offset, uint8_t* destination, size_t requested,
                                           size_t* bytesRead);
  static PdfStatus writeResolverObjectStore(void* context, const uint8_t* source, size_t requested,
                                            size_t* bytesWritten);
  static bool lookupCachedObjectStream(void* context, uint32_t objectStreamNumber, uint32_t* objectCount,
                                       uint64_t* first, uint64_t* size);
  static PdfStatus prepareCachedObjectStream(void* context, uint64_t requiredBytes, bool* cacheCleared);
  static void publishCachedObjectStream(void* context, uint32_t objectStreamNumber, uint32_t objectCount,
                                        uint64_t first, uint64_t size);
  static PdfStatus readPreparedFontStore(void* context, uint64_t offset, uint8_t* destination, size_t requested,
                                         size_t* bytesRead);
  static PdfStatus writePreparedFontStore(void* context, const uint8_t* source, size_t requested, size_t* bytesWritten);
  static PdfStatus observePreparedFontRecord(void* context, uint32_t ordinal, const void* record, size_t recordSize);
  ...
  PdfStatus beginResumeReferenceValidation(PdfObjectReference reference, bool allowNull, bool validateObjectStream);
  ...
  PdfStatus finishCurrentOutlineNode(const PdfStatus& destinationStatus, const PdfResolvedDestination& destination);
  PdfStatus finishCurrentAnnotation(const PdfStatus& destinationStatus, const PdfResolvedDestination& destination);
  ...
  void rememberNamedDestination(const PdfRawDestination& named, const PdfResolvedDestination& destination);
  ...
  PdfStatus appendPreparedFontEncodingDifferences(uint16_t dictionaryIndex, PreparedContentRuntime* runtime);
  ...
  uint8_t* fontNavigationSnapshotFieldBytes(NavigationWorkspace* workspace, size_t offset, size_t* contiguousBytes);
