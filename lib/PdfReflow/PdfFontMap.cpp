...
int32_t estimatedScalarWidth(const uint32_t scalar) {
  ...
  if (scalar < 0x20U || scalar == 0x7fU || scalar == 0x200bU || scalar == 0x200cU || scalar == 0x200dU ||
      scalar == 0x2060U || (scalar >= 0x0300U && scalar <= 0x036fU) || (scalar >= 0x1dc0U && scalar <= 0x1dffU) ||
      (scalar >= 0x20d0U && scalar <= 0x20ffU) || (scalar >= 0xfe00U && scalar <= 0xfe0fU) ||
      (scalar >= 0xfe20U && scalar <= 0xfe2fU) || (scalar >= 0xe0100U && scalar <= 0xe01efU)) {
    return 0;
  }
  if (scalar == 0x00a0U || scalar == 0x1680U || (scalar >= 0x2000U && scalar <= 0x200aU) || scalar == 0x202fU ||
...
PdfStatus PdfFontMap::decodeNext(const uint8_t* const source, const size_t sourceLength, PdfDecodedGlyph* const glyph) {
  ...
  if (materialized()) {
    ...
    if (status.ok() || (cid_ && !estimatedIdentityFallback()) || status.error != PdfError::UnsupportedEncoding) {
      return status;
    }
    uint32_t scalar = 0;
