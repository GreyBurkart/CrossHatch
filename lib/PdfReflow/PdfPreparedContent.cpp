...
PdfStatus PdfPreparedContentStreams::beginSequence(const PdfEncodedContentStream* const streams, const uint8_t count,
...
PdfStatus PdfPreparedContentStreams::beginInternal(const PdfEncodedContentStream* const streams, const uint8_t count,
                                                   const PdfByteStore decodedStore, const uint64_t existingBytes,
                                                   const bool resetStore, const bool sequenceMode,
                                                   const bool prefixSeparator, const PdfStreamDecodeLimits limits) {
  streams_ = nullptr;
  streamCount_ = 0;
  decodedStore_ = {};
  limits_ = {};
  ...
  limits_ = limits;
  limits_.maxExpandedBytes = std::min<uint64_t>(limits_.maxExpandedBytes, PdfLimits::MaxExpandedRequiredStreamBytes);
  limits_.maxExpansionRatio = std::min<uint16_t>(limits_.maxExpansionRatio, PdfLimits::MaxExpansionRatio);
...
PdfStepResult PdfPreparedContentStreams::step(PdfWorkBudget& budget) {
  ...
  while (true) {
    ...
    if (phase_ == Phase::BeginStream) {
      ...
      if (sequenceMode_ && !separatorWrittenForCurrent_ && (prefixSeparator_ || streamIndex_ != 0)) {
      ...
      if (streams_[streamIndex_].security != nullptr) {
        status = streams_[streamIndex_].security->openStream(encoded, streams_[streamIndex_].reference, &encoded);
      }
      if (status) {
    ...
    if (phase_ == Phase::FinalizeSources) {
      if (finalizeIndex_ >= streamCount_) {
        if (sequenceMode_ && !sequenceSource_.valid()) {
          ...
          const PdfStatus status = pdfInitializeByteRange(decodedSource, 0, decodedBytes_, &sequenceRange_);
          if (!status) {
            return fail(status);
          }
