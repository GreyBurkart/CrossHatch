...
PdfStatus validateDefaultDecodeParameters(const PdfObjectArena& arena, const uint16_t dictionaryIndex,
  ...
  if (parameters.predictor < 10U || parameters.predictor > 15U || !isPngBitsPerComponent(parameters.bitsPerComponent) ||
      decodeParameters == nullptr) {
...
PdfStatus PdfStreamDecoder::begin(const PdfByteSource& source, const PdfByteSink& sink, const PdfStreamFilter* filters,
  ...
  if (decodeParameters != nullptr && decodeParameters->predictor != 1U) {
    if (decodeParameters->predictor < 10U || decodeParameters->predictor > 15U || decodeParameters->columns == 0U ||
        decodeParameters->colors == 0U || !isPngBitsPerComponent(decodeParameters->bitsPerComponent) || !hasFlate_) {
      phase_ = Phase::Failed;
...
PdfStatus PdfStreamDecoder::applyPredictor(uint8_t* const bytes, const size_t inputLength, size_t* const outputLength) {
  ...
  *outputLength = inputLength;
  ...
  for (size_t read = 0; read < inputLength; ++read) {
    ...
    predictorRowPosition_ =
        predictorRowPosition_ == predictorRowBytes_ ? 0U : static_cast<uint16_t>(predictorRowPosition_ + 1U);
  }
