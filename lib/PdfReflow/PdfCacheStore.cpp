...
PDF_CACHE_STORE_NOINLINE PdfStatus loadManifestSlot(const PdfCacheIo& io, const char* const path,
  ...
  PdfCacheHandle handle{};
  PdfStatus status = io.open(io.context, path, PdfCacheOpenMode::Read, &handle);
  if (!status) {
    return status;
  }
  ...
  state->valid = true;
  state->sourceMatches = pdfSourceIdentityEqual(state->manifest.source, expectedSource);
  return PdfStatus::success();
}

PDF_CACHE_STORE_NOINLINE PdfStatus loadCheckpointSlot(const PdfCacheIo& io, const char* const path,
                                                      const PdfSourceIdentity& expectedSource,
  ...
  if (status && !metadata.directory && !metadata.symlinkLike) {
    CacheHandleSource source{&io, handle, metadata.size};
    status = pdfDecodeBuildCheckpoint(source.source(), &state->checkpoint);
  } else if (status) {
    status = PdfStatus::failure(PdfError::Malformed);
  }
  status = closeHandle(io, &handle, status);
  ...
  state->valid = true;
  state->sourceMatches = pdfSourceIdentityEqual(state->checkpoint.source, expectedSource);
  return PdfStatus::success();
}

PDF_CACHE_STORE_NOINLINE PdfStatus verifyCheckpointFile(const PdfCacheIo& io, const char* const path,
...
PDF_CACHE_STORE_NOINLINE PdfStatus removeGenerationPath(const PdfCacheIo& io, const char* const root,
                                                        const uint32_t generation) {
  char path[PDF_CACHE_PATH_CAPACITY];
  const int length = std::snprintf(path, sizeof(path), "%s/gen_%lu", root, static_cast<unsigned long>(generation));
  if (length < 0 || static_cast<size_t>(length) >= sizeof(path)) {
    return PdfStatus::failure(PdfError::LimitExceeded);
  }
  const PdfStatus status = io.remove(io.context, path, true);
  return !status && status.error == PdfError::InvalidOffset ? PdfStatus::success() : status;
}

PdfStatus collectGeneration(void* const context, const PdfCacheDirEntry& entry) {
...
PdfStatus PdfCacheStore::loadManifestSlots(const PdfSourceIdentity& expectedSource,
  ...
  for (uint8_t index = 0; index < 2; ++index) {
    ...
    if (!status) {
      if (status.error == PdfError::InvalidOffset || isRecoverableSlotError(status.error)) {
        resetInPlace(state);
        continue;
      }
      return status;
    ...
    if (state.sourceMatches &&
        (!selection->selected || pdfCacheSequenceNewer(state.manifest.sequence, selection->manifest.sequence))) {
...
PdfStatus PdfCacheStore::loadCheckpointSlots(const PdfSourceIdentity& expectedSource,
  ...
  for (uint8_t index = 0; index < 2; ++index) {
    ...
    if (state.sourceMatches &&
        (!selection->selected || pdfCacheSequenceNewer(state.checkpoint.sequence, selection->checkpoint.sequence))) {
      selection->selected = true;
      selection->selectedSlot = index == 0 ? PdfCacheSlot::A : PdfCacheSlot::B;
...
PdfStatus PdfCacheStore::listGenerations(PdfCacheGenerationList* const generations) const {
  if (!io_.valid() || generations == nullptr) {
    return PdfStatus::failure(PdfError::InvalidArgument);
  }
  *generations = {};
  const PdfStatus status = io_.list(io_.context, root_, collectGeneration, generations);
  return !status && status.error == PdfError::InvalidOffset ? PdfStatus::success() : status;
}

PdfStatus PdfCacheStore::removeGeneration(const uint32_t generation) const {
...
PdfStatus PdfCacheStore::cleanupUnreferencedGenerations(const PdfCacheManifestSelection& manifests) const {
  if (!io_.valid()) {
    return PdfStatus::failure(PdfError::InvalidArgument);
  }
