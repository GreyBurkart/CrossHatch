  ...
  *length = static_cast<uint16_t>(token.length);
  if (token.length != 0) {
    const uint8_t* const source =
        token.reserved[0] != 0 ? stringTokenBuffer_ : reinterpret_cast<const uint8_t*>(token.bytes);
  ...
  if (depth_ != 0 && frames_[depth_ - 1].dictionary && frames_[depth_ - 1].expectingKey) {
    ...
    if (status.ok()) {
      ...
      skipDictionaryValue_ =
          ...
          (skipUnusedPageResources_ && (keyEquals("ExtGState", 9) || keyEquals("ColorSpace", 10) ||
                                        keyEquals("Pattern", 7) || keyEquals("Shading", 7) || keyEquals("ProcSet", 7) ||
                                        keyEquals("Properties", 10) || keyEquals("Widths", 6)));
    }
    return status;
  }
