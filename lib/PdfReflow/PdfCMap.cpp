...
bool PdfCMap::observingRecords() const { return workspace_.spill.write != nullptr && workspace_.spill.read == nullptr; }
...
PdfStatus PdfCMap::addRecord(const PdfCMapRecord& record) {
  ...
  const bool ordered = !hasPreviousRecord_ || (key >= previousRecordKey_ &&
  ...
  if (!codeSpaceOnly()) {
    if (observingRecords()) {
      ...
      const PdfStatus status = workspace_.spill.write(workspace_.spill.context, mappingCount_, &record, sizeof(record));
      if (!status.ok()) {
...
PdfStatus PdfCMap::addSequential(const uint32_t first, const uint32_t last, const uint8_t sourceLength,
  ...
  if (!decodeStatus.ok() || scalar > std::numeric_limits<uint32_t>::max() - (count - 1) ||
      !pdfIsUnicodeScalar(lastScalar) || (scalar < 0xD800 && lastScalar >= 0xD800)) {
