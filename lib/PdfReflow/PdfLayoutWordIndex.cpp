...
PdfStatus pdfEncodeLayoutCacheBindingTrailer(const PdfLayoutCacheBinding& binding,
...
PdfStatus pdfComputeLayoutCacheBinding(const PdfByteSource& sectionCache, PdfLayoutCacheBinding* const binding) {
  ...
  if (!sectionCache.valid() || binding == nullptr || sectionCache.size < PDF_LAYOUT_CACHE_BINDING_TRAILER_BYTES ||
      sectionCache.size > kMaximumBoundCacheBytes) {
    return PdfStatus::failure(PdfError::InvalidArgument);
  }
  *binding = {};
  uint8_t trailer[PDF_LAYOUT_CACHE_BINDING_TRAILER_BYTES];
  const PdfStatus status = pdfReadExact(sectionCache, sectionCache.size - sizeof(trailer), trailer, sizeof(trailer));
  if (!status) {
    return status;
  }
...
PdfStatus pdfBindLayoutWordIndex(const PdfByteSource& source, const PdfLayoutWordIndexPatchSink& patch,
  ...
  for (uint16_t firstPage = 0; firstPage < info.pageCount;) {
    ...
    for (uint16_t index = 0; index < count; ++index) {
      aggregateCrc =
          pdfCacheCrc32(encoded + static_cast<size_t>(index) * PDF_LAYOUT_WORD_INDEX_RECORD_BYTES, 36, aggregateCrc);
    }
    firstPage = static_cast<uint16_t>(firstPage + count);
  }
...
bool pdfLayoutWordIndexMatchesSectionCache(const PdfLayoutWordIndexInfo& info, const PdfLayoutCacheBinding& binding) {
...
PdfStatus pdfReadValidatedLayoutWordRanges(const PdfByteSource& source, const PdfLayoutWordIndexInfo& info,
  ...
  for (uint16_t decoded = 0; decoded < count;) {
    const uint16_t batchCount = std::min<uint16_t>(kReadBatchRecords, static_cast<uint16_t>(count - decoded));
    PdfStatus status = readEncodedRangeBatch(source, static_cast<uint16_t>(firstPage + decoded), batchCount, encoded);
    if (!status) {
      return status;
    }
    for (uint16_t index = 0; index < batchCount; ++index) {
...
PdfStatus pdfReadValidatedLayoutPageRecords(const PdfByteSource& source, const PdfLayoutWordIndexInfo& info,
  ...
  for (uint16_t index = 0; index < count; ++index) {
    ...
    status = decodeRange(encoded + static_cast<size_t>(index) * PDF_LAYOUT_WORD_INDEX_RECORD_BYTES, &ignored, nullptr,
                         &pages[index]);
