...
PdfStatus PdfPageModel::appendStoredText(const uint8_t* const text, const size_t length) {
  ...
  const size_t resident =
      textLength_ < workspace_.textCapacity ? std::min(length, workspace_.textCapacity - textLength_) : 0U;
  if (resident != 0) {
  ...
  if (spilled != 0) {
    ...
    const PdfStatus status =
        workspace_.spillText(workspace_.spillTextContext, textLength_ + resident, text + resident, spilled);
    if (!status) {
      return status;
    }
  }
...
PdfStatus PdfPageModel::beginOverflowTextRun(const PdfTextRun& run, uint16_t* const runIndex) {
  ...
  const int64_t absoluteBaselineDifference = baselineDifference < 0 ? -baselineDifference : baselineDifference;
...
PdfStatus PdfPageModel::appendOverflowText(const uint8_t* const text, const size_t length) {
  ...
  const bool currentUnicodeHyphen =
      length == 3U && text[0] == 0xE2U && text[1] == 0x80U && (text[2] == 0x90U || text[2] == 0x91U);
  const bool previousSoftHyphen =
      textTailLength_ >= 2U && textTail_[textTailLength_ - 2U] == 0xC2U && textTail_[textTailLength_ - 1U] == 0xADU;
  const bool previousUnicodeHyphen =
      textTailLength_ >= 3U && textTail_[textTailLength_ - 3U] == 0xE2U && textTail_[textTailLength_ - 2U] == 0x80U &&
      (textTail_[textTailLength_ - 1U] == 0x90U || textTail_[textTailLength_ - 1U] == 0x91U);
  const bool inferredWordJoin =
      currentSoftHyphen || currentUnicodeHyphen || previousSoftHyphen || previousUnicodeHyphen;
  ...
  if (runCount_ != 0) {
    ...
    if (duplicateOverlayOffset_ != UINT16_MAX) {
      ...
      if (duplicateGeometry && duplicateEnd <= previous.textLength &&
        ...
        duplicateOverlayOffset_ =
            duplicateEnd == previous.textLength ? UINT16_MAX : static_cast<uint16_t>(duplicateEnd);
        rollbackPendingText();
        return PdfStatus::success();
      }
      duplicateOverlayOffset_ = UINT16_MAX;
    ...
    const bool exactPaintDuplicate =
        ...
        std::memcmp(workspace_.text + previous.textOffset, workspace_.text + current.textOffset, current.textLength) ==
            0;
    ...
    const int64_t lineHeight = std::max(static_cast<int64_t>(previous.yMax) - previous.yMin,
    ...
    const int64_t absoluteBaselineDifference = baselineDifference < 0 ? -baselineDifference : baselineDifference;
    const bool overlappingInlineFragment = minimumHeight > 0 && verticalOverlap > 0 &&
                                           verticalOverlap * 2 >= minimumHeight &&
    ...
    const bool wideAbsolutePosition = (current.flags & PdfTextPositionReset) != 0 && baselineGap > lineHeight * 4;
    ...
    const bool isolatedSoftHyphen = current.textLength == 2U && workspace_.text[current.textOffset] == 0xC2U &&
                                    workspace_.text[current.textOffset + 1U] == 0xADU;
    const bool isolatedUnicodeHyphen =
        current.textLength == 3U && workspace_.text[current.textOffset] == 0xE2U &&
        workspace_.text[current.textOffset + 1U] == 0x80U &&
        (workspace_.text[current.textOffset + 2U] == 0x90U || workspace_.text[current.textOffset + 2U] == 0x91U);
    if ((previous.flags & mergeRelevantFlags) == (current.flags & mergeRelevantFlags) &&
        (current.flags & PdfTextSemanticBoundary) == 0U && (weightDifference == 0U || overlappingInlineFragment) &&
        (!wideAbsolutePosition || isolatedSoftHyphen || isolatedUnicodeHyphen) &&
        (absoluteBaselineDifference <= 65536 || overlappingInlineFragment) && baselineGap <= maximumMergeGap &&
      ...
      const bool hiddenTextLayer = (previous.flags & PdfTextHidden) != 0U && (current.flags & PdfTextHidden) != 0U;
      ...
      const bool currentJoinsPunctuation = currentFirst == '-' || currentFirst == ',' || currentFirst == '.' ||
                                           ...
                                           currentFirst == '?' || currentFirst == ')' || currentFirst == ']' ||
                                           currentFirst == '}';
      const bool previousSoftHyphen = previousTextEnd >= 2U && workspace_.text[previousTextEnd - 2U] == 0xC2U &&
                                      workspace_.text[previousTextEnd - 1U] == 0xADU;
      const bool previousUnicodeHyphen =
          previousTextEnd >= 3U && workspace_.text[previousTextEnd - 3U] == 0xE2U &&
          workspace_.text[previousTextEnd - 2U] == 0x80U &&
          (workspace_.text[previousTextEnd - 1U] == 0x90U || workspace_.text[previousTextEnd - 1U] == 0x91U);
      const bool previousJoinsWord = previousSoftHyphen || previousUnicodeHyphen ||
                                     (workspace_.text[current.textOffset - 1U] == '-' &&
                                      (baselineGap <= lineHeight / 4 || (currentFirst >= '0' && currentFirst <= '9')));
      ...
      if (currentAllCapsFragment && (current.textLength != 1U || (currentFirst != 'A' && currentFirst != 'I')) &&
          baselineGap <= lineHeight / 4 && (current.flags & PdfTextArrayTightContinuation) != 0) {
      ...
      const bool charactersNeedSpace =
          workspace_.text[current.textOffset - 1U] != ' ' && workspace_.text[current.textOffset] != ' ';
      ...
      while (previousWordStart > previous.textOffset && asciiLetter(workspace_.text[previousWordStart - 1U])) {
        --previousWordStart;
      }
      const uint64_t previousWordLength = previousTextEnd - previousWordStart;
      const bool repeatedWordGap =
          (current.flags & PdfTextExplicitWhitespace) != 0 && (current.flags & PdfTextArrayTightContinuation) == 0 &&
          baselineGap > wordGap && previousWordLength >= 4U && current.textLength >= 2U &&
          asciiLetter(workspace_.text[current.textOffset]) && asciiLetter(workspace_.text[current.textOffset + 1U]) &&
      ...
      const bool positionedWordContinuation = asciiLetter(workspace_.text[previousTextEnd - 1U]) &&
                                              asciiLetter(currentFirst) && baselineGap <= positionedContinuationGap;
