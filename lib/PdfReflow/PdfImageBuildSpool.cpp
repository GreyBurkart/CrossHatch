...
bool validRecord(const PdfDeferredImageRecord& record) {
  ...
  return record.reference.objectNumber != 0 && record.streamLength != 0 && record.width != 0 && record.height != 0 &&
         ...
         (!record.hasAuxiliary || (record.auxiliaryReference.objectNumber != 0 && record.auxiliaryStreamLength != 0 &&
...
PdfObjectReference decodeReference(const uint8_t* const input) { return {readLe32(input), readLe16(input + 4)}; }
...
bool validEncodedRecord(const uint8_t* input) {
  ...
  return reference.objectNumber != 0 && readLe64(input + 8) != 0 && readLe32(input + 44) != 0 &&
         readLe32(input + 48) != 0 && filterCount <= PdfLimits::MaxFiltersPerStream &&
         auxiliaryFilterCount <= PdfLimits::MaxFiltersPerStream && paletteBytes <= PDF_IMAGE_BUILD_PALETTE_BYTES &&
         tagLength != 0 && static_cast<uint64_t>(tagOffset) + tagLength <= UINT32_MAX &&
         (!hasAuxiliary || (auxiliaryReference.objectNumber != 0 && readLe64(input + 32) != 0 &&
                            readLe32(input + 52) != 0 && readLe32(input + 56) != 0));
}
