...
uint32_t rotateLeft(const uint32_t value, const uint8_t bits) { return (value << bits) | (value >> (32U - bits)); }
...
PdfStatus findUnique(const PdfObjectArena& arena, const uint16_t dictionaryIndex, const char* key, uint16_t* valueIndex,
                     bool* found = nullptr) {
...
PdfStatus PdfSecurity::initializeEmptyPassword(const PdfObjectArena& arena, const uint16_t dictionaryIndex,
  ...
  if (!status || standardFilter >= arena.valueCount || arena.values[standardFilter].kind != PdfValueKind::Dictionary) {
    return PdfStatus::failure(PdfError::Encrypted, trailer.encryptionReference.objectNumber);
  }
  ...
  if (!status ||
      (found && (arena.values[optional].kind != PdfValueKind::Boolean || !arena.values[optional].booleanValue))) {
    return PdfStatus::failure(PdfError::Encrypted, trailer.encryptionReference.objectNumber);
  }
...
PdfStatus PdfSecurity::readStream(void* const context, const uint64_t offset, uint8_t* const destination,
  ...
  const PdfStatus status =
      security.stream_.source.readAt(security.stream_.source.context, offset, destination, requested, &read);
