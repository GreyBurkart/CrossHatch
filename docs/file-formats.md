...
These formats describe the SD-card cache files under `/.crosspoint/`, including
the EPUB cache under `/.crosspoint/epub_<hash>/` and the PDF reflow cache under
`/.crosspoint/pdf_<hash>/`. All POD fields are written in the ESP32
little-endian representation used by `Serialization.h`; strings are
...
- `[20-23]` exact byte length of the unchanged version 66 section `.bin`
  prefix
...
version 66 section-cache prefix followed by one 16-byte binding trailer:

- `[0-3]` magic `PWIB`
- `[4-7]` exact version 66 prefix length
...
indexes into the existing version 66 section `.bin` in four-record (32-byte)
...
does not perform a full eager rehash of the version 66 prefix, so an arbitrary
