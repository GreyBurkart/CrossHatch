...
- PDF reading (Xteink X4 Classic only for now; X3/X4, X4 Pro, and Sticky builds leave it out until the firmware fits their app partition): text-based and OCRed PDFs open from Browse Files and are reflowed with your reader font, size, margins, and orientation, like EPUBs. The first open prepares a reading cache on the SD card (cancellable and resumable). Outlines become chapters, internal links work, and meaningful images are kept. Bookmarks, clippings, progress, covers on Home and sleep screens, moves, trash, and web uploads all support PDFs. Image-only scans without a text layer, password-protected PDFs, fixed-page zoom, KOReader sync, and Nearby sync or transfer are not supported for PDFs. See [docs/pdf-support.md](docs/pdf-support.md). Ported from [CrossPDF](https://github.com/davemessew/CrossPDF) (MIT).
